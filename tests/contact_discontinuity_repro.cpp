...
#define POLYFEM_TEST_DIR "."
	...
	auto touches_suspect_dict = [&](const auto& dict) {
		for (auto vid : dict.primary_vertex_ids()) {
			if (vid >= 0 && (int)vid < nv && suspect.count((int)vid)) return true;
		}
		return false;
	};
	...
	auto dict_hashes = [](const auto& dict) {
		...
		for (int i = 0; i < dict.size(); ++i) out.push_back(dict[i].get_typed_hash());
		return out;
	};
	...
	std::cout << "-- vertex_collisions --\n";
	{
		...
		for (auto& [k, _] : L.collisions.vertex_collisions) keys.insert(k);
		for (auto& [k, _] : H.collisions.vertex_collisions) keys.insert(k);
		for (auto k : keys) {
			...
			if (!((dl && touches_suspect_dict(*dl)) || (dh && touches_suspect_dict(*dh)))) continue;
	...
	std::cout << "-- edge_edge_collisions --\n";
	{
		...
		for (auto& [k, _] : L.collisions.edge_edge_collisions) keys.insert(k);
		for (auto& [k, _] : H.collisions.edge_edge_collisions) keys.insert(k);
		for (auto k : keys) {
	...
	auto pt_dtype_str = [](ipc::PointTriangleDistanceType t) {
		switch (t) {
			case ipc::PointTriangleDistanceType::P_T0: return "P_T0";
			case ipc::PointTriangleDistanceType::P_T1: return "P_T1";
			case ipc::PointTriangleDistanceType::P_T2: return "P_T2";
			case ipc::PointTriangleDistanceType::P_E0: return "P_E0";
			case ipc::PointTriangleDistanceType::P_E1: return "P_E1";
			case ipc::PointTriangleDistanceType::P_E2: return "P_E2";
			case ipc::PointTriangleDistanceType::P_T:  return "P_T";
			default: return "AUTO";
		}
	};
	auto pe_dtype_str = [](ipc::PointEdgeDistanceType t) {
		switch (t) {
			case ipc::PointEdgeDistanceType::P_E0: return "P_E0";
			case ipc::PointEdgeDistanceType::P_E1: return "P_E1";
			case ipc::PointEdgeDistanceType::P_E:  return "P_E";
			default: return "AUTO";
		}
	};
	...
	{
		std::set<ipc::index_t> keys;
		for (auto& [k, _] : L.collisions.face_collisions) keys.insert(k);
		for (auto& [k, _] : H.collisions.face_collisions) keys.insert(k);
		for (auto k : keys) {
			...
			for (size_t qi = 0; qi < nq; ++qi) {
				...
				for (int i = 0; i < ns; ++i) {
					...
					if (i < s_hi) {
						const auto& c = (*dh)[i];
						if (r.name.empty()) { r.name = c.name(); r.hash = c.get_typed_hash(); }
	...
	std::sort(frows.begin(), frows.end(), [](const FaceRow& a, const FaceRow& b) {
		...
		if (a_flip != b_flip) return a_flip;
		return std::max(std::abs(a.val_hi - a.val_lo), a.dgnorm) >
		       std::max(std::abs(b.val_hi - b.val_lo), b.dgnorm);
	});
	...
	for (int i = 0; i < M; ++i) {
		...
		std::cout << r.key;
		for (int p = (int)r.key.size(); p < 30; ++p) std::cout << ' ';
		std::cout << r.name;
		for (int p = (int)r.name.size(); p < 10; ++p) std::cout << ' ';
