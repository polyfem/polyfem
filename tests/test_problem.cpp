...
TEST_CASE("franke 2d", "[problem]")
{
	...
	//rhs
	{
		...
		probl->rhs(assembler, "Laplacian", pts, 1, other);
		Eigen::MatrixXd diff = (other - rhs);

		REQUIRE(diff.array().abs().maxCoeff() < 1e-10);

		rhs += k * k * fx;
		probl->rhs(assembler, "Helmholtz", pts, 1, other);
		diff = (other - rhs);
...
TEST_CASE("elasticity 2d", "[problem]")
{
	...
	//rhs
	{
		...
		probl->rhs(assembler, "LinearElasticity", pts, 1, other);
		Eigen::MatrixXd diff = (other - rhs);

		REQUIRE(diff.array().abs().maxCoeff() < 1e-10);

		probl->rhs(assembler, "HookeLinearElasticity", pts, 1, other);
		diff = (other - rhs);
	...
	//rhs
	{
		...
		probl->rhs(assembler, "SaintVenant", pts, 1, other);
		Eigen::MatrixXd diff = (other - rhs);
	...
	//rhs
	{
		...
		probl->rhs(assembler, "NeoHookean", pts, 1, other);
		Eigen::MatrixXd diff = (other - rhs);
