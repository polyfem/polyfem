...
namespace poly_fem
{
	...
	public:
		...
		void set_bc(const int size, const std::vector< ElementBases > &bases, const Mesh &mesh, const std::vector< LocalBoundary > &local_boundary, const std::vector<int> &bounday_nodes, const int resolution,  const Problem &problem, Eigen::MatrixXd &rhs) const;
