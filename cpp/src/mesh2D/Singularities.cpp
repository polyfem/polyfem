	...
	GEO::Mesh &M, const Eigen::VectorXi &V, const Eigen::MatrixX2i &E, double t)
{
	...
	for (int i = 0; i < V.size(); ++i) { singulars.push_back(V(i)); }
	for (int i = 0; i < E.size(); ++i) { singulars.push_back(E.data()[i]); }
	...
	std::vector<bool> is_singular(M.vertices.nb(), false);
	for (int v : singulars) { is_singular[v] = true; }
	...
	for (index_t f = 0, old_num_facets = M.facets.nb(); f < old_num_facets; ++f) {
		...
		if (nv == 1) {
			if (sf == 3) { create_midfacet_point = false; }
			else { create_midfacet_point = true; }
		} else if (nv == 2) {
			create_midfacet_point = false;
		} else {
			if (sf == 3 || sf == 4) { create_midfacet_point = true; }
			else { create_midfacet_point = false; }
		}
