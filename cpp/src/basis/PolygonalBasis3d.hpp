...
namespace poly_fem
{
	...
	public:
		...
		/// @param[in]     values              { Per-element shape functions for the PDE, evaluated
		///                                    over the element, used for the system matrix assembly
		///                                    (used for linear reproduction) }
		/// @param[in]     gvalues             { Per-element shape functions for the geometric
		///                                    mapping, evaluated over the element (get boundary of
		///                                    the polygon) }
		/// @param[in,out] bases               { List of the different basis (shape functions) used
		///                                    to discretize the PDE }
		/// @param[in]     gbases              { List of the different basis used to discretize the
		///                                    geometry of the mesh }
		/// @param[in]     poly_face_to_data   { Additional data computed for faces at the interface
		///                                    with a polygon }
		/// @param         mapped_boundary     { Map element id > (V, E) triangle mesh surface
		///                                    formed by the image of the collocation points trough
		///                                    the geometric mapping of the boundary faces }
		///
