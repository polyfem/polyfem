...
namespace poly_fem
{
	...
	Biharmonic::Biharmonic(const Eigen::MatrixXd &centers, const Eigen::MatrixXd &samples, const Eigen::MatrixXd &rhs)
	: centers_(centers)
	{
		compute(samples, rhs);
	}
	...
	void Biharmonic::basis(const int local_index, const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) const
	{
	...
	void Biharmonic::grad(const int local_index, const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) const
	{
		const Eigen::MatrixXd w = weights_.col(local_index);
		const long end = w.size()-1;
	...
	void Biharmonic::compute(const Eigen::MatrixXd &samples, const Eigen::MatrixXd &rhs)
	{
