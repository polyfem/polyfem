...
template<typename T>
Eigen::MatrixXd theta2(T &t) { return t * (2 * t - 1); }

template<typename T>
Eigen::MatrixXd theta3(T &t) { return 4 * t * (1 - t); }
...
template<typename T>
Eigen::MatrixXd dtheta2(T &t) { return -1+4*t; }

template<typename T>
Eigen::MatrixXd dtheta3(T &t) { return 4-8*t; }
...
int prepare_q1(const Mesh2D &mesh, std::vector< BasisValue > &values, std::vector< LocalBoundary > &local_boundary, std::vector< int > &bounday_nodes, std::map<int, InterfaceData> &poly_edge_to_data)
{
	Eigen::MatrixXd node;
	...
	for(int e = 0; e < mesh.n_elements(); ++e)
	{
		...
		if(n_el_vertices == 4)
		{
			...
			{
				int f2 = mesh.switch_face(index).face;
				if (f2 < 0) {
				...
				} else if (mesh.n_element_vertices(f2) > 4) {
					InterfaceData data;
                    data.element_id = index.face;
					switch (j) {
						case 1: data.flag = InterfaceData::TOP_FLAG; break;
						case 2: data.flag = InterfaceData::LEFT_FLAG; break;
						case 3: data.flag = InterfaceData::BOTTOM_FLAG; break;
						case 0: data.flag = InterfaceData::RIGHT_FLAG; break;
						default: assert(false);
					}
					auto idx = index;
                    data.node_id.push_back(index.vertex);
                    data.node_id.push_back((idx = mesh.switch_vertex(idx)).vertex);
                    data.node_id.push_back((idx = mesh.switch_vertex(idx)).vertex);
                    data.node_id.push_back((idx = mesh.switch_vertex(idx)).vertex);
                    data.local_indices.push_back((j+0)%4);
                    data.local_indices.push_back((j+1)%4);
                    data.local_indices.push_back((j+2)%4);
                    data.local_indices.push_back((j+3)%4);
                    data.vals.push_back(1);
                    data.vals.push_back(1);
                    data.vals.push_back(1);
                    data.vals.push_back(1);
                    poly_edge_to_data[index.edge] = data;
				}
...
int prepare_q2(const Mesh2D &mesh, std::vector< BasisValue > &values, std::vector< LocalBoundary > &local_boundary, std::vector< int > &bounday_nodes, std::map<int, InterfaceData> &poly_edge_to_data)
{
	Eigen::MatrixXd node;
	values.resize(mesh.n_elements());
...
void tri_basis_grad(const int discr_order, const int local_index, const Eigen::MatrixXd &uv, Eigen::MatrixXd &val)
{
...
{
	//TODO fill poly_edge_to_data when elements are adjacent to a polygon
	bounday_nodes.clear();
	...
	int n_bases;
	if(discr_order == 1) {
		n_bases = prepare_q1(mesh, values, local_boundary, bounday_nodes, poly_edge_to_data);
	} else if(discr_order == 2) {
		n_bases = prepare_q2(mesh, values, local_boundary, bounday_nodes, poly_edge_to_data);
	} else {
		assert(false);
	}
	...
	{
		const BasisValue &bv = values[f];
		ElementBases &b=bases[f];
		...
		else if(n_el_vertices == 3)
		{
			...
			for (int j = 0; j < n_loc_bases; ++j)
			{
				...
				b.bases[j].set_basis([discr_order, j](const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) { tri_basis_basis(discr_order, j, uv, val); });
				b.bases[j].set_grad( [discr_order, j](const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) {  tri_basis_grad(discr_order, j, uv, val); });
			}
		}
		else
		{
