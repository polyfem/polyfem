...
namespace poly_fem
{
	...
	HarmonicBasis::HarmonicBasis(const Eigen::MatrixXd &centers, const Eigen::MatrixXd &samples, const Eigen::MatrixXd &rhs)
	: centers_(centers)
	{
		compute(samples, rhs);
	}
	...
	void HarmonicBasis::basis(const int local_index, const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) const
	{
	...
	void HarmonicBasis::grad(const int local_index, const Eigen::MatrixXd &uv, Eigen::MatrixXd &val) const
	{
		const Eigen::MatrixXd w = weights_.col(local_index);
		const long end = w.size()-1;
	...
	void HarmonicBasis::compute(const Eigen::MatrixXd &samples, const Eigen::MatrixXd &rhs)
	{
