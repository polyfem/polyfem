...
#include <polyfem/EnableWarnings.hpp>
		...
		Eigen::MatrixXd unflatten(const TVector &x) { return polyfem::unflatten(x, state.mesh->dimension()); }
		void reduced_to_full_unflattened(const TVector &reduced, Eigen::MatrixXd &full_unflattened);

		double barrier_stiffness() const { return _barrier_stiffness; }
		const TVector &full_x_prev() const { return _full_x_prev; }
	...
	private:
		...
		TVector _full_x_prev;          ///< @brief Displaced vertices at the start of the time-step.
