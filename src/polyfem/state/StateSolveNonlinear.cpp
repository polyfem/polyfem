...
namespace polyfem
{
	...
	void State::init_nonlinear_tensor_solve(Eigen::MatrixXd &sol, const double t, const bool init_time_integrator)
	{
		...
		const std::vector<std::shared_ptr<Form>> forms = solve_data.init_forms(
			...
			args["contact"].contains("static_friction_coefficient") ? args["contact"]["static_friction_coefficient"] : friction_coefficient,
			args["contact"].contains("kinetic_friction_coefficient") ? args["contact"]["kinetic_friction_coefficient"] : friction_coefficient,
			args["contact"]["epsv"],
