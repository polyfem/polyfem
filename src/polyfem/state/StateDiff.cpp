...
namespace polyfem
{
	...
	void State::cache_transient_adjoint_quantities(const int current_step)
	{
		...
		diff_cached.push_back({gradu_h, gradu_h_next, sol, 1, solve_data.contact_form ? solve_data.contact_form->get_constraint_set() : ipc::Constraints(), solve_data.friction_form ? solve_data.friction_form->get_friction_constraint_set() : ipc::FrictionConstraints()});
	}
	...
	void State::compute_force_hessian(StiffnessMatrix &hessian, StiffnessMatrix &hessian_prev, const int bdf_order)
	{
		...
		else
		{
			...
			compute_force_hessian_nonlinear(hessian, hessian_prev, bdf_order);
		}
	}

	void State::compute_force_hessian_nonlinear(StiffnessMatrix &hessian, StiffnessMatrix &hessian_prev, const int bdf_order)
	{
		...
		if (solve_data.friction_form)
		{
			...
			if (problem->is_time_dependent())
			{
				hessian_prev /= solve_data.time_integrator->acceleration_scaling();
			}
		}
	...
	void State::compute_shape_derivative_rhs_term(const Eigen::MatrixXd &solution, const Eigen::MatrixXd &adjoint_sol, Eigen::VectorXd &term)
	{
		...
		utils::maybe_parallel_for(n_elements, [&](int start, int end, int thread_id) {
			...
			for (int e = start; e < end; ++e)
			{
				...
				for (int q = 0; q < local_storage.da.size(); ++q)
				{
					const double value = dot(p.row(q), rhs_function.row(q)) * local_storage.da(q);
					for (auto &v : gvals.basis_values)
					{
						local_storage.vec.block(v.global[0].index * mesh->dimension(), 0, mesh->dimension(), 1) += value * v.grad_t_m.row(q).transpose();
					}
				}
			}
		});
		...
		Eigen::VectorXd &one_form)
	{
		...
		if (j.depend_on_u())
		{
			solve_data.elastic_form->foce_material_derivative(sol, sol, adjoint_sol, one_form);
		}
	}
	...
	void State::dJ_friction_transient(const IntegrableFunctional &j, double &one_form)
	{
		...
		for (int t = time_steps; t > 0; --t)
		{
			...
			auto force = -ipc::compute_friction_force(collision_mesh, collision_mesh.vertices_at_rest(), surface_solution_prev, surface_solution, friction_constraint_set, solve_data.contact_form->dhat(), solve_data.contact_form->barrier_stiffness(), solve_data.friction_form->epsv_dt(), 0, true);
	...
	void State::dJ_damping_transient(const IntegrableFunctional &j, Eigen::VectorXd &one_form)
	{
		...
		for (int i = time_steps; i > 0; --i)
		{
			...
			solve_data.damping_form->foce_material_derivative(diff_cached[i].u, diff_cached[i - 1].u, -adjoint_p[i], damping_term);
	...
	void State::dJ_material_static(const IntegrableFunctional &j, Eigen::VectorXd &one_form)
	{
		...
		solve_data.elastic_form->foce_material_derivative(sol, sol, adjoint_sol, elasticity_term);
	...
	void State::dJ_material_transient(const IntegrableFunctional &j, Eigen::VectorXd &one_form)
	{
		...
		for (int i = time_steps; i > 0; --i)
		{
			...
			solve_data.elastic_form->foce_material_derivative(diff_cached[i].u, diff_cached[i - 1].u, -adjoint_p[i], elasticity_term);
