...
namespace polyfem
{
	...
	void State::init_homogenization_solve(const double t)
	{
		...
		std::shared_ptr<NLHomoProblem> homo_problem = std::make_shared<NLHomoProblem>(
			ndof,
			macro_strain_constraint,
			*this, t, forms, solve_data.al_form, solve_symmetric_flag, polysolve::linear::Solver::create(args["solver"]["linear"], logger()), characteristic_length, characteristic_force, pure_mass);
