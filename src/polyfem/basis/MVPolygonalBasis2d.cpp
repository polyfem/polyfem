	...
	namespace basis
	{
			...
			std::map<int, Eigen::MatrixXd> &mapped_boundary)
		{
			...
			for (int e = 0; e < mesh.n_elements(); ++e)
			{
				...
				poly_quadr.get_quadrature(polygon, std::max(quadrature_order, Quadrature::stiffness_poly_order(1, 2)), tmp_quadrature);

				Quadrature tmp_mass_quadrature;
				poly_quadr.get_quadrature(polygon, std::max(mass_quadrature_order, Quadrature::mass_poly_order(1, 2)), tmp_mass_quadrature);
