	...
	Eigen::MatrixXd &nodes_position)
{
	...
	return build_bases(mesh, quadrature_order, discr_orders, serendipity, has_polys, is_geom_bases, bases, local_boundary, poly_edge_to_data, primitive_to_node, nodes_position);
}
	...
	Eigen::MatrixXd &nodes_position)
{
	...
	nodes_position.setZero(nodes.n_nodes(), 2);
	for (int i = 0; i < nodes_position.rows(); i++)
	{
		nodes_position.row(i) = nodes.node_position(i);
	}
