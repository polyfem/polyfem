	...
	namespace basis
	{
										  ...
										  const std::map<int, InterfaceData> &poly_edge_to_data, std::map<int, Eigen::MatrixXd> &mapped_boundary)
		{
			...
			for (int e = 0; e < mesh.n_elements(); ++e)
			{
				...
				poly_quadr.get_quadrature(collocation_points, quadrature_order > 0 ? quadrature_order : Quadrature::stiffness_poly_order(2, 2), tmp_quadrature);

				Quadrature tmp_mass_quadrature;
				poly_quadr.get_quadrature(collocation_points, mass_quadrature_order > 0 ? mass_quadrature_order : Quadrature::mass_poly_order(2, 2), tmp_mass_quadrature);
