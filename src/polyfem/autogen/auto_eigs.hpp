...
namespace polyfem::autogen
{
	...
	template <typename T>
	void eigs_2d(const MatrixN<T> &m, VectorN<T> &res)
	{
		res.resize(2);
	...
	template <typename T>
	void eigs_3d(const MatrixN<T> &m, VectorN<T> &res)
	{
		res.resize(3);
		const auto helper_0 = m(0, 0) + m(1, 1);
		...
		const auto helper_11 = 0.33333333333333331 * ((helper_10 >= 1.0) ? (T(0)) : ((helper_10 <= -1.0) ? (T(M_PI)) : (acos(helper_10))));
