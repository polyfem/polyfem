...
namespace polyfem::varform
{
	...
	QuadratureOrders VarForm::n_boundary_samples() const
	{
		...
		return n_boundary_samples(space.disc_orders.maxCoeff(), space.geometry->disc_orders.maxCoeff());
	}
	...
	const FESpace &VarForm::primary_space() const
	{
		log_and_throw_error("Variational formulation {} does not expose a primary FE space.", name());
	}

	const VarFormBoundaryState &VarForm::boundary_state() const
	{
		log_and_throw_error("Variational formulation {} does not expose boundary state.", name());
	}

	const assembler::Assembler &VarForm::primary_assembler() const
	{
		log_and_throw_error("Variational formulation {} does not expose a primary assembler.", name());
	}

	const assembler::Mass &VarForm::mass_assembler() const
	{
		log_and_throw_error("Variational formulation {} does not expose a mass assembler.", name());
	}

	const assembler::AssemblyValsCache &VarForm::assembly_cache() const
	{
		log_and_throw_error("Variational formulation {} does not expose an assembly cache.", name());
	}

	const assembler::AssemblyValsCache &VarForm::mass_assembly_cache() const
	{
		log_and_throw_error("Variational formulation {} does not expose a mass assembly cache.", name());
	}

	const StiffnessMatrix &VarForm::mass_matrix() const
	{
		log_and_throw_error("Variational formulation {} does not expose a mass matrix.", name());
	}

	const ipc::CollisionMesh &VarForm::collision_mesh() const
	{
		log_and_throw_error("Variational formulation {} does not expose a collision mesh.", name());
	}

	const mesh::Obstacle &VarForm::get_obstacle() const
	{
		log_and_throw_error("Variational formulation {} does not expose an obstacle.", name());
	}

	void VarForm::initial_solution(Eigen::MatrixXd &, const InitialConditionOverride *) const
	{
		log_and_throw_error("Variational formulation {} does not expose an initial solution.", name());
	}

	void VarForm::initial_velocity(Eigen::MatrixXd &, const InitialConditionOverride *) const
	{
		log_and_throw_error("Variational formulation {} does not expose an initial velocity.", name());
	}

	void VarForm::initial_acceleration(Eigen::MatrixXd &, const InitialConditionOverride *) const
	{
		log_and_throw_error("Variational formulation {} does not expose an initial acceleration.", name());
	}

	void VarForm::build_stiffness_matrix(StiffnessMatrix &stiffness) const
	{
		const FESpace &space = primary_space();
	...
	void VarForm::get_elements(Eigen::MatrixXi &elements) const
	{
	...
	bool VarForm::is_problem_linear() const
	{
	...
	void VarForm::set_lame_parameters(const Eigen::VectorXd &lambda, const Eigen::VectorXd &mu)
	{
	...
	void VarForm::set_friction_coefficient(const double coefficient)
	{
		args["contact"]["friction_coefficient"] = coefficient;
		invalidate_after_parameter_update();
	}

	void VarForm::set_damping_coefficients(const double psi, const double phi)
	{
	...
	void VarForm::set_dirichlet_boundary(const int boundary_id, const int time_step, const Eigen::VectorXd &value)
	{
	...
	void VarForm::set_dirichlet_nodes(const Eigen::VectorXi &input_nodes, const Eigen::MatrixXd &values)
	{
		auto tensor_problem = std::dynamic_pointer_cast<assembler::GenericTensorProblem>(problem);
		if (!tensor_problem)
	...
	void VarForm::set_pressure_boundary(const int boundary_id, const int time_step, const double value)
	{
		auto tensor_problem = std::dynamic_pointer_cast<assembler::GenericTensorProblem>(problem);
		if (!tensor_problem)
