...
namespace polyfem::utils
{
	...
	SpareMatrixCache::SpareMatrixCache(const size_t size)
		: size_(size)
	{
	...
	SpareMatrixCache::SpareMatrixCache(const size_t rows, const size_t cols)
	...
	SpareMatrixCache::SpareMatrixCache(const SpareMatrixCache &other)
	{
		init(other);
	}

	void SpareMatrixCache::init(const size_t size)
	{
	...
	void SpareMatrixCache::init(const SpareMatrixCache &other)
	{
	...
	void SpareMatrixCache::set_zero()
	{
		tmp_.setZero();
		mat_.setZero();
	...
	void SpareMatrixCache::add_value(const int e, const int i, const int j, const double value)
	{
		if (mapping().empty())
		{
	...
	polyfem::StiffnessMatrix SpareMatrixCache::get_matrix(const bool compute_mapping)
	{
		prune();
	...
	SpareMatrixCache SpareMatrixCache::operator+(const SpareMatrixCache &a) const
	{
	...
	void SpareMatrixCache::operator+=(const SpareMatrixCache &o)
	{
	...
	Eigen::SparseMatrix<double> lump_matrix(const Eigen::SparseMatrix<double> &M)
	{
		...
		{
			for (StiffnessMatrix::InnerIterator it(M, k); it; ++it)
			{
