...
namespace polyfem
{
	namespace utils
	{
		...
		public:
			...
			static void quadrature_for_prism_face(int index, int order, int order1, const int gid, const mesh::Mesh &mesh, Eigen::MatrixXd &uv, Eigen::MatrixXd &points, Eigen::VectorXd &weights);
			...
			static bool boundary_quadrature(const mesh::LocalBoundary &local_boundary, const int order, const int order1, const mesh::Mesh &mesh, const bool skip_computation, Eigen::MatrixXd &uv, Eigen::MatrixXd &points, Eigen::MatrixXd &normals, Eigen::VectorXd &weights, Eigen::VectorXi &global_primitive_ids);
			// compute quadrature for one boundary facet, uv are local (ref) values, samples are global coordinates
			static bool boundary_quadrature(const mesh::LocalBoundary &local_boundary, const int order, const int order1, const mesh::Mesh &mesh, const int i, const bool skip_computation, Eigen::MatrixXd &uv, Eigen::MatrixXd &points, Eigen::MatrixXd &normal, Eigen::VectorXd &weights);
		};
	} // namespace utils
} // namespace polyfem
