		...
		for (int i = 0; i < n_constrained_quantaties; ++i)
		{
			...
			projected_quantities.col(i) = constrained_L2_projection(
				...
				m.state.solve_data.contact_form
					...
					: 1.0,
				m.state.args["contact"]["use_convergent_formulation"] ? m.state.args["contact"]["use_area_weighting"] : false,
				m.state.args["contact"]["use_convergent_formulation"] ? m.state.args["contact"]["use_improved_max_operator"] : false,
				m.state.args["contact"]["use_convergent_formulation"] ? m.state.args["contact"]["use_physical_barrier"] : false,
