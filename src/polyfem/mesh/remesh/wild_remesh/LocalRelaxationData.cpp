		...
		const bool contact_enabled)
	{
		...
		std::vector<std::shared_ptr<solver::Form>> forms;
		{
			...
			forms = solve_data.init_forms(
				...
				assembly_vals_cache, assembly_vals_cache, state.args["solver"]["advanced"]["jacobian_threshold"], state.args["solver"]["advanced"]["check_inversion"], state.args["solver"]["advanced"]["quadrature_refinement"],
				// Body form
