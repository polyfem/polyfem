...
namespace polyfem::legacy
{
	...
	void State::init(const json &p_args_in, const bool strict_validation)
	{
		...
		if (optimization_enabled)
		{
			if (is_contact_enabled())
			{
				if (!args["contact"]["use_high_order_formulation"] && !args["contact"]["use_gcp_formulation"] && !args["contact"]["use_convergent_formulation"] && !args["contact"]["use_offset_formulation"])
				{
