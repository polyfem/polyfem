...
namespace polyfem::legacy
{
	...
	NLHomoProblem::TVector NLHomoProblem::full_to_reduced(const TVector &full, const Eigen::MatrixXd &disp_grad) const
	{
		...
		reduced.head(dof1) = NLProblem::full_to_reduced(full - polyfem::io::Evaluator::generate_linear_field(state_.n_bases, state_.mesh_nodes, disp_grad));
	...
	NLHomoProblem::TVector NLHomoProblem::reduced_to_full(const TVector &reduced) const
	{
		...
		return NLProblem::reduced_to_full(reduced.head(dof1)) + polyfem::io::Evaluator::generate_linear_field(state_.n_bases, state_.mesh_nodes, disp_grad);
	}
	...
	Eigen::MatrixXd NLHomoProblem::constraint_grad() const
	{
		...
		Eigen::MatrixXd X = polyfem::io::Evaluator::get_bases_position(state_.n_bases, state_.mesh_nodes);
