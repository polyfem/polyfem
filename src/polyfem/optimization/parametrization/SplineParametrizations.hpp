...
namespace polyfem::solver
{
	...
	private:
		...
		// TODO: This is a workaround. I want to make inverse_eval() const,
		// but right now it's used as initializer/cache layer so not possible.
		mutable bool invoked_inverse_eval_ = false;
		mutable std::shared_ptr<BSplineParametrization2D> spline_;
	};

	// Comment out because this class is half-baked. Some methods are dummy.
	//
	...
	public:
		...
		BoundedBiharmonicWeights2Dto3D(const int num_control_vertices, const int num_vertices, const varform::DifferentiableVarForm &varform, const bool allow_rotations);
	...
	private:
		...
		// but right now it's used as initializer/cache layer so not possible.
		mutable bool invoked_inverse_eval_ = false;
		mutable Eigen::VectorXd y_start;
		mutable Eigen::MatrixXd control_points_;
		mutable Eigen::MatrixXd bbw_weights_;
		mutable Eigen::MatrixXd boundary_bbw_weights_;
