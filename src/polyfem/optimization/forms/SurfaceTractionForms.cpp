...
namespace polyfem::solver
{
	namespace
	{
			...
			Eigen::MatrixXi &can_collide_cache)
		{
			...
			io::OutGeometryData::extract_boundary_mesh(varform.get_mesh(), varform.primary_space().n_bases, varform.primary_space().basis_list(), varform.boundary_state().total_local_boundary,
			...
			for (const auto &lb : varform.boundary_state().total_local_boundary)
			{
				...
				bool has_samples = utils::BoundarySampler::boundary_quadrature(lb, varform.n_boundary_samples(), varform.get_mesh(), false, uv, points, normals, weights, global_primitive_ids);

				if (!has_samples)
					continue;

				const basis::ElementBases &bs = varform.primary_space().basis_list()[e];
				const basis::ElementBases &gbs = varform.primary_space().geometry_basis_list()[e];

				vals.compute(e, varform.get_mesh().is_volume(), points, bs, gbs);

				for (int i = 0; i < lb.size(); ++i)
				{
					const int primitive_global_id = lb.global_primitive_id(i);
					const auto nodes = bs.local_nodes_for_primitive(primitive_global_id, varform.get_mesh());
			...
			{
				displacement_map.resize(node_positions.rows(), varform.primary_space().n_bases);
...

		{
			...
			cs_.build(collision_mesh, displaced_surface, dhat, dmin, ipc::create_broad_phase(ipc::BroadPhaseMethod::HASH_GRID).get());
	...
	// 	j.set_j([formulation, power, &varform = std::as_const(varform_)](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::MatrixXd &lambda, const Eigen::MatrixXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const json &params, Eigen::MatrixXd &val) {
	...
	// 			// varform.primary_assembler().compute_stress_grad_multiply_vect(el_id, local_pts.row(q), pts.row(q), grad_u_q, Eigen::MatrixXd::Zero(1, grad_u_q.cols()), stress, grad_unused);
	// 			varform.primary_assembler().compute_stress_grad_multiply_vect(OptAssemblerData(t, dt, el_id, local_pts.row(q), pts.row(q), grad_u_q), Eigen::MatrixXd::Zero(1, grad_u_q.cols()), stress, grad_unused);
	// 			traction_force = displaced_normal * stress;
	// 			val(q) = pow(traction_force.squaredNorm(), power / 2.);
	// 		}
	// 	});

	// 	auto dj_dgradx = [formulation, power, &varform = std::as_const(varform_)](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::MatrixXd &lambda, const Eigen::MatrixXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const json &params, Eigen::MatrixXd &val) {
	...
	// 	auto dj_dgradu = [formulation, power, &varform = std::as_const(varform_)](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::MatrixXd &lambda, const Eigen::MatrixXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const json &params, Eigen::MatrixXd &val) {
	// 		val.setZero(grad_u.rows(), grad_u.cols());
	// 		int el_id = params["elem"];
	// 		const double dt = varform.get_problem().is_time_dependent() ? varform.get_args()["time"]["dt"].get<double>() : 0;
	// 		const double t = params["t"];
	...
	// 			reference_normal = reference_normals.row(q);
	// 			displaced_normal = compute_displaced_normal(reference_normal, grad_x, grad_u_local).transpose();
	// 			// varform.primary_assembler().compute_stress_grad_multiply_vect(el_id, local_pts.row(q), pts.row(q), grad_u_q, displaced_normal, stress, vect_mult_dstress);
	// 			varform.primary_assembler().compute_stress_grad_multiply_vect(OptAssemblerData(t, dt, el_id, local_pts.row(q), pts.row(q), grad_u_q), displaced_normal, stress, vect_mult_dstress);
	// 			traction_force = displaced_normal * stress;
	...
	// 		{
	// 			vector2matrix(grad_u.row(q), grad_u_q);
	// 			varform.primary_assembler().compute_stress_grad_multiply_vect(el_id, local_pts.row(q), pts.row(q), grad_u_q, displaced_normals.row(q), stress, normal_dstress);
	...
	// 	auto dj_du = [formulation, power, &varform = std::as_const(varform_)](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::MatrixXd &lambda, const Eigen::MatrixXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const json &params, Eigen::MatrixXd &val) {
	...
	// 		{
	// 			vector2matrix(grad_u.row(q), grad_u_q);
	// 			varform.primary_assembler().compute_stress_grad_multiply_mat(el_id, local_pts.row(q), pts.row(q), grad_u_q, Eigen::MatrixXd::Zero(grad_u_q.rows(), grad_u_q.cols()), stress, grad_unused);
	// 			traction_force = displaced_normals.row(q) * stress;
	...
	// 	dim_ = varform_.get_mesh().dimension();
	// 	for (const auto &lb : varform_.total_local_boundary)
	// 	{
	// 		const int e = lb.element_id();
	// 		const basis::ElementBases &bs = varform_.bases[e];

	// 		for (int i = 0; i < lb.size(); i++)
	// 		{
	// 			const int global_primitive_id = lb.global_primitive_id(i);
	// 			const auto nodes = bs.local_nodes_for_primitive(global_primitive_id, varform_.get_mesh());
	// 			if (ids_.size() != 0 && ids_.find(varform_.get_mesh().get_boundary_id(global_primitive_id)) == ids_.end())
	// 				continue;
	...
	// 	active_nodes_mat_.resize(varform_.collision_mesh.full_ndof(), active_nodes_set.size());
	...
	// 	epsv_ = varform_.args["contact"]["epsv"];
	// 	dhat_ = varform_.args["contact"]["dhat"];
	// 	friction_coefficient_ = varform_.args["contact"]["friction_coefficient"];
	...
	// 	double barrier_stiffness = varform_.solve_data.contact_form->weight();

	// 	const ipc::CollisionMesh &collision_mesh = varform_.collision_mesh;
	// 	Eigen::MatrixXd displaced_surface = collision_mesh.displace_vertices(utils::unflatten(varform_.diff_cached.u(time_step), dim_));

	// 	Eigen::MatrixXd forces = Eigen::MatrixXd::Zero(collision_mesh.ndof(), 1);
	// 	forces -= barrier_stiffness
	// 			  * varform_.solve_data.contact_form->barrier_potential().gradient(
	// 				  varform_.diff_cached.collision_set(time_step), collision_mesh, displaced_surface);
	// 	if (varform_.solve_data.friction_form && time_step > 0)
	// 	{
	// 		Eigen::MatrixXd surface_solution_prev = collision_mesh.vertices(utils::unflatten(varform_.diff_cached.u(time_step - 1), collision_mesh.dim()));
	// 		Eigen::MatrixXd surface_solution = collision_mesh.vertices(utils::unflatten(varform_.diff_cached.u(time_step), collision_mesh.dim()));
	// 		const Eigen::MatrixXd surface_velocities = (surface_solution - surface_solution_prev) / varform_.solve_data.time_integrator->dt();

	// 		forces += varform_.solve_data.friction_form->friction_potential().force(
	// 			varform_.diff_cached.friction_collision_set(time_step),
	// 			collision_mesh,
	// 			collision_mesh.rest_positions(),
	// 			surface_solution_prev,
	// 			surface_velocities,
	// 			varform_.solve_data.contact_form->barrier_potential(),
	// 			varform_.solve_data.contact_form->barrier_stiffness(),
	// 			varform_.solve_data.friction_form->epsv());
	// 	}
	...
	// 			   * varform_.solve_data.contact_form->barrier_potential().hessian(
	// 				   varform_.diff_cached.collision_set(time_step), collision_mesh, displaced_surface, false);
	// 	if (varform_.solve_data.friction_form && time_step > 0)
	// 	{
	// 		Eigen::MatrixXd surface_solution_prev = collision_mesh.vertices(utils::unflatten(varform_.diff_cached.u(time_step - 1), collision_mesh.dim()));
	// 		Eigen::MatrixXd surface_solution = collision_mesh.vertices(utils::unflatten(varform_.diff_cached.u(time_step), collision_mesh.dim()));
	...
	// 		const double dv_du = -1 / varform_.solve_data.time_integrator->dt();
	// 		hessian += varform_.solve_data.friction_form->friction_potential().force_jacobian(
	// 			varform_.diff_cached.friction_collision_set(time_step),
	// 			collision_mesh,
	// 			collision_mesh.rest_positions(),
	// 			surface_solution_prev,
	...
	// 		const double dv_du = -1 / varform_.solve_data.time_integrator->dt();
	// 		hessian_prev += varform_.solve_data.friction_form->friction_potential().force_jacobian(
	// 			varform_.diff_cached.friction_collision_set(time_step),
	// 			collision_mesh,
	// 			collision_mesh.rest_positions(),
	// 			surface_solution_prev,
	...
	// 		hessian_prev += dv_du
	// 						* varform_.solve_data.friction_form->friction_potential().force_jacobian(
	// 							varform_.diff_cached.friction_collision_set(time_step),
	// 							collision_mesh,
	// 							collision_mesh.rest_positions(),
	// 							surface_solution_prev,
	...
	// 						* varform_.solve_data.contact_form->barrier_potential().shape_derivative(
	// 							varform_.diff_cached.collision_set(time_step), collision_mesh, displaced_surface);
	// 	if (varform_.solve_data.friction_form && time_step > 0)
	// 	{
	// 		Eigen::MatrixXd surface_solution_prev = collision_mesh.vertices(utils::unflatten(varform_.diff_cached.u(time_step - 1), collision_mesh.dim()));
	...
	// 		shape_derivative += varform_.solve_data.friction_form->friction_potential().force_jacobian(
	// 			varform_.diff_cached.friction_collision_set(time_step),
	// 			collision_mesh,
	// 			collision_mesh.rest_positions(),
	// 			surface_solution_prev,
	...
	// 	grads = varform_.basis_nodes_to_gbasis_nodes * grads;
	// 	grads = AdjointTools::map_node_to_primitive_order(varform_, grads);
	...
	// 		for (const auto &varform : param_map->get_varforms())
	// 		{
	// 			if (varform.get() != &varform_)
	// 				continue;
	...
	ProxyContactForceForm::ProxyContactForceForm(
		...
		: StaticForm(variable_to_simulations),
		  ...
		  dhat_(dhat),
		  dmin_(0),
		  barrier_potential_(dhat, 1.0, true)
	{
		...
		if (varform_->get_problem().is_time_dependent())
		{
			int time_steps = varform_->get_args()["time"]["time_steps"].get<int>() + 1;
	...
	void ProxyContactForceForm::build_collision_mesh()
	{
		...
		std::vector<Eigen::Triplet<double>> displacement_map_entries;
		io::OutGeometryData::extract_boundary_mesh(varform_->get_mesh(), varform_->primary_space().n_bases, varform_->primary_space().basis_list(), varform_->boundary_state().total_local_boundary,
		...
		Eigen::VectorXd weights;
		Eigen::VectorXi global_primitive_ids;
		for (const auto &lb : varform_->boundary_state().total_local_boundary)
		{
			const int e = lb.element_id();
			bool has_samples = utils::BoundarySampler::boundary_quadrature(lb, varform_->n_boundary_samples(), varform_->get_mesh(), false, uv, points, normals, weights, global_primitive_ids);

			if (!has_samples)
				continue;

			const basis::ElementBases &bs = varform_->primary_space().basis_list()[e];
			const basis::ElementBases &gbs = varform_->primary_space().geometry_basis_list()[e];

			vals.compute(e, varform_->get_mesh().is_volume(), points, bs, gbs);

			for (int i = 0; i < lb.size(); ++i)
			{
				const int primitive_global_id = lb.global_primitive_id(i);
				const auto nodes = bs.local_nodes_for_primitive(primitive_global_id, varform_->get_mesh());
				const int boundary_id = varform_->get_mesh().get_boundary_id(primitive_global_id);
		...
		if (!displacement_map_entries.empty())
		{
			displacement_map.resize(node_positions_.rows(), varform_->primary_space().n_bases);
			displacement_map.setFromTriplets(displacement_map_entries.begin(), displacement_map_entries.end());
		}
		...
		Eigen::MatrixXi boundary_edges_alt(0, 2), boundary_triangles_alt(0, 3);
		{
			...
			if (varform_->get_mesh().is_volume())
			{
				for (int i = 0; i < boundary_triangles.rows(); ++i)
				{
					bool on_surface = true;
	...
	const ipc::NormalCollisions &ProxyContactForceForm::get_or_compute_collision_set(const int time_step, const Eigen::MatrixXd &displaced_surface) const
	{
		if (!collision_set_indicator_(time_step))
		{
			collision_sets_[time_step]->build(
				collision_mesh_, displaced_surface, dhat_, dmin_, ipc::create_broad_phase(broad_phase_method_).get());
	...
	Eigen::VectorXd ProxyContactForceForm::compute_adjoint_rhs_step(const int time_step, const Eigen::VectorXd &x, const varform::DifferentiableVarForm &varform, const DiffCache &diff_cache) const
	{
		...
		// logger().trace("u norm {}", varform_.diff_cached.u(time_step).norm());

		// Eigen::VectorXd G;
		// fd::finite_gradient(
		// 	varform_.diff_cached.u(time_step), [this, time_step](const Eigen::VectorXd &x_) {
	...
	void ProxyContactForceForm::compute_partial_gradient_step(const int time_step, const Eigen::VectorXd &x, Eigen::VectorXd &gradv) const
	{
		...
		gradv = weight() * variable_to_simulations_.apply_parametrization_jacobian(ParameterType::Shape, *varform_, x, [this, time_step, &x]() {
			...
			// 	compute_forward_collision_mesh_quantities(varform_, boundary_ids_, collision_mesh_, n, e, t, d_m, i_o_s, c_c_c);
			...
			// 		varform_.diff_cached.u(time_step),
			// 		barrier_potential_); },
			...
			return AdjointTools::map_node_to_primitive_order(*varform_, grads);
		});
	}
