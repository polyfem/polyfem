...
namespace polyfem::solver
{
	...
	{
	public:
		TargetForm(const VariableToSimulationGroup &variable_to_simulations, std::shared_ptr<const legacy::State> state, std::shared_ptr<const DiffCache> diff_cache, const json &args)
		...
		void set_reference(const std::shared_ptr<const legacy::State> &target_state, std::shared_ptr<const DiffCache> target_diff_cache, const std::set<int> &reference_cached_body_ids); // target is another simulation solution
	...
	protected:
		...
		std::shared_ptr<const legacy::State> target_state_;
	...
	{
	public:
		SDFTargetForm(const VariableToSimulationGroup &variable_to_simulations, std::shared_ptr<const legacy::State> state, std::shared_ptr<const DiffCache> diff_cache, const json &args)
			: SpatialIntegralForm(variable_to_simulations, std::move(state), std::move(diff_cache), args)
		{
			set_integral_type(SpatialIntegralType::Surface);
	...
	{
	public:
		MeshTargetForm(const VariableToSimulationGroup &variable_to_simulations, std::shared_ptr<const legacy::State> state, std::shared_ptr<const DiffCache> diff_cache, const json &args)
			: SpatialIntegralForm(variable_to_simulations, std::move(state), std::move(diff_cache), args)
		{
			set_integral_type(SpatialIntegralType::Surface);
	...
	{
	public:
		NodeTargetForm(std::shared_ptr<const legacy::State> state, std::shared_ptr<const DiffCache> diff_cache, const VariableToSimulationGroup &variable_to_simulations, const json &args);
		NodeTargetForm(std::shared_ptr<const legacy::State> state, std::shared_ptr<const DiffCache> diff_cache, const VariableToSimulationGroup &variable_to_simulations, const std::vector<int> &active_nodes_, const Eigen::MatrixXd &target_vertex_positions_);
	...
	{
	public:
		BarycenterTargetForm(const VariableToSimulationGroup &variable_to_simulations, const json &args, const std::shared_ptr<legacy::State> &state1, std::shared_ptr<const DiffCache> diff_cache1, const std::shared_ptr<legacy::State> &state2, std::shared_ptr<const DiffCache> diff_cache2);
	...
	{
	public:
		MinTargetDistForm(const VariableToSimulationGroup &variable_to_simulations, const std::vector<int> &steps, const Eigen::VectorXd &target, const json &args, const std::shared_ptr<legacy::State> &state, std::shared_ptr<const DiffCache> diff_cache);
		virtual ~MinTargetDistForm() = default;

		Eigen::MatrixXd compute_adjoint_rhs(const Eigen::VectorXd &x, const legacy::State &state, const DiffCache &diff_cache) const override;
