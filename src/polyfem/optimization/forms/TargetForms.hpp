...
namespace polyfem::solver
{
	...
	public:
		...
		void set_reference(const std::shared_ptr<const varform::DifferentiableVarForm> &target_varform, std::shared_ptr<const DiffCache> target_diff_cache, const std::set<int> &reference_cached_body_ids);
	...
	protected:
		...
		std::shared_ptr<const varform::DifferentiableVarForm> target_varform_;
	...
	{
	public:
		SDFTargetForm(const VariableToSimulationGroup &variable_to_simulations, std::shared_ptr<const varform::DifferentiableVarForm> varform, std::shared_ptr<const DiffCache> diff_cache, const json &args)
			: SpatialIntegralForm(variable_to_simulations, std::move(varform), std::move(diff_cache), args)
		{
			set_integral_type(SpatialIntegralType::Surface);
	...
	{
	public:
		MeshTargetForm(const VariableToSimulationGroup &variable_to_simulations, std::shared_ptr<const varform::DifferentiableVarForm> varform, std::shared_ptr<const DiffCache> diff_cache, const json &args)
			: SpatialIntegralForm(variable_to_simulations, std::move(varform), std::move(diff_cache), args)
		{
			set_integral_type(SpatialIntegralType::Surface);
	...
	{
	public:
		BarycenterTargetForm(const VariableToSimulationGroup &variable_to_simulations, const json &args, const std::shared_ptr<varform::DifferentiableVarForm> &varform1, std::shared_ptr<const DiffCache> diff_cache1, const std::shared_ptr<varform::DifferentiableVarForm> &varform2, std::shared_ptr<const DiffCache> diff_cache2);
	...
	{
	public:
		MinTargetDistForm(const VariableToSimulationGroup &variable_to_simulations, const std::vector<int> &steps, const Eigen::VectorXd &target, const json &args, const std::shared_ptr<varform::DifferentiableVarForm> &varform, std::shared_ptr<const DiffCache> diff_cache);
		virtual ~MinTargetDistForm() = default;

		Eigen::MatrixXd compute_adjoint_rhs(const Eigen::VectorXd &x, const varform::DifferentiableVarForm &varform, const DiffCache &diff_cache) const override;
