...
namespace polyfem::solver
{
	...
	IntegrableFunctional TargetForm::get_integral_functional() const
	{
		...
		if (target_varform_)
		{
			...
			auto j_func = [this](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::VectorXd &lambda, const Eigen::VectorXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const IntegrableFunctional::ParameterType &params, Eigen::MatrixXd &val) {
				...
				target_varform_->primary_space().geometry_basis_list()[e_ref].eval_geom_mapping(local_pts, pts_ref);

				Eigen::MatrixXd u_ref, grad_u_ref;
				const Eigen::VectorXd &sol_ref = target_diff_cache_->u(target_varform_->get_problem().is_time_dependent() ? params.step : 0);
				io::Evaluator::interpolate_at_local_vals(target_varform_->get_mesh(), target_varform_->get_problem().is_scalar(), target_varform_->primary_space().basis_list(), target_varform_->primary_space().geometry_basis_list(), e_ref, local_pts, sol_ref, u_ref, grad_u_ref);
		...
		{
			if (target_disp.size() == varform_->get_mesh().dimension())
			{
				auto j_func = [this](const Eigen::MatrixXd &local_pts, const Eigen::MatrixXd &pts, const Eigen::MatrixXd &u, const Eigen::MatrixXd &grad_u, const Eigen::VectorXd &lambda, const Eigen::VectorXd &mu, const Eigen::MatrixXd &reference_normals, const assembler::ElementAssemblyValues &vals, const IntegrableFunctional::ParameterType &params, Eigen::MatrixXd &val) {
					val.setZero(u.rows(), 1);
	...
	void TargetForm::set_reference(const std::shared_ptr<const varform::DifferentiableVarForm> &target_varform, std::shared_ptr<const DiffCache> target_diff_cache, const std::set<int> &reference_cached_body_ids)
	{
		...
		int ref_count = 0;
		for (int e = 0; e < target_varform_->primary_space().basis_list().size(); ++e)
		{
			int body_id = target_varform_->get_mesh().get_body_id(e);
		...
		{
			int body_id = varform_->get_mesh().get_body_id(e);
			if (reference_cached_body_ids.size() > 0 && reference_cached_body_ids.count(body_id) == 0)
				continue;
	...
	void SDFTargetForm::solution_changed_step(const int time_step, const Eigen::VectorXd &x)
	{
		...
		const auto &gbases = varform_->primary_space().geometry_basis_list();
		const int actual_dim = varform_->get_problem().is_scalar() ? 1 : dim;

		auto storage = utils::create_thread_storage(LocalThreadScalarStorage());
		utils::maybe_parallel_for(varform_->boundary_state().total_local_boundary.size(), [&](int start, int end, int thread_id) {
			...
			{
				const auto &lb = varform_->boundary_state().total_local_boundary[lb_id];
				...
				for (int i = 0; i < lb.size(); i++)
				{
					...
					utils::BoundarySampler::boundary_quadrature(lb, varform_->n_boundary_samples(), varform_->get_mesh(), i, false, uv, points, normal, weights);

					assembler::ElementAssemblyValues &vals = local_storage.vals;
					vals.compute(e, varform_->get_mesh().is_volume(), points, bases[e], gbases[e]);
	...
	void SDFTargetForm::set_bspline_target(const Eigen::MatrixXd &control_points, const Eigen::VectorXd &knots, const double delta)
	{
		...
		delta_ = delta;
		if ((dim != 2) || (varform_->get_mesh().dimension() != 2))
	...
	void SDFTargetForm::set_bspline_target(const Eigen::MatrixXd &control_points, const Eigen::VectorXd &knots_u, const Eigen::VectorXd &knots_v, const double delta)
	{
		...
		dim = control_points.cols();
		delta_ = delta;
		if ((dim != 3) || (varform_->get_mesh().dimension() != 3))
		...
		: StaticForm(variable_to_simulations), varform_(std::move(varform)), diff_cache_(std::move(diff_cache))
	{
		...
		for (int s = 0; s < data.rows(); s++)
		{
			...
			for (int v = 0; v < varform_->primary_space().mesh_nodes->n_nodes(); v++)
			{
				min_dist = std::min(min_dist, (varform_->primary_space().mesh_nodes->node_position(v) - node).norm());
				if ((varform_->primary_space().mesh_nodes->node_position(v) - node).norm() < args["tolerance"])
				{
	...
	Eigen::VectorXd NodeTargetForm::compute_adjoint_rhs_step(const int time_step, const Eigen::VectorXd &x, const varform::DifferentiableVarForm &varform, const DiffCache &diff_cache) const
	{
		...
		if (&varform == varform_.get())
		{
			...
			{
				const RowVectorNd cur_pos = varform_->primary_space().mesh_nodes->node_position(v) + disp.segment(v * dim, dim).transpose();
	...
	void NodeTargetForm::compute_partial_gradient_step(const int time_step, const Eigen::VectorXd &x, Eigen::VectorXd &gradv) const
	{
		...
		gradv = weight() * variable_to_simulations_.apply_parametrization_jacobian(ParameterType::Shape, *varform_, x, [this]() {
		...
		: StaticForm(variable_to_simulations)
	{
		...
		{
			tmp_args["dim"] = d;
			center1.push_back(std::make_unique<PositionForm>(variable_to_simulations, varform1, diff_cache1, tmp_args));
			center2.push_back(std::make_unique<PositionForm>(variable_to_simulations, varform2, diff_cache2, tmp_args));
		}
	}
	...
	Eigen::VectorXd BarycenterTargetForm::compute_adjoint_rhs_step(const int time_step, const Eigen::VectorXd &x, const varform::DifferentiableVarForm &varform, const DiffCache &diff_cache) const
	{
		...
		for (int d = 0; d < dim; d++)
		{
			...
			term += (2 * value) * (center1[d]->compute_adjoint_rhs_step(time_step, x, varform, diff_cache) - center2[d]->compute_adjoint_rhs_step(time_step, x, varform, diff_cache));
		}
		return term * weight();
	}
		...
		: AdjointForm(variable_to_simulations), steps_(steps), target_(target)
	{
		...
		{
			tmp_args["dim"] = d;
			objs.push_back(std::make_unique<PositionForm>(variable_to_simulations, varform, diff_cache, tmp_args));
		}
	...
	Eigen::MatrixXd MinTargetDistForm::compute_adjoint_rhs(const Eigen::VectorXd &x, const varform::DifferentiableVarForm &varform, const DiffCache &diff_cache) const
	{
		...
		for (int s : steps_)
		{
			...
			for (int d = 0; d < objs.size(); d++)
			{
				...
				grads[i].col(d) = objs[d]->compute_adjoint_rhs_step(s, x, varform, diff_cache);
			}
		...
		Eigen::MatrixXd terms = Eigen::MatrixXd::Zero(varform.primary_space().ndof(), diff_cache.size());
		i = 0;
		for (int s : steps_)
		{
