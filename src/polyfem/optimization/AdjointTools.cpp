...
namespace polyfem::solver
{
	namespace
	{
		...
		int get_bdf_order(const polyfem::varform::VarForm &state)
		{
			if (state.get_args()["time"]["integrator"].is_string())
				return 1;
			if (state.get_args()["time"]["integrator"]["type"] == "ImplicitEuler")
				return 1;
			if (state.get_args()["time"]["integrator"]["type"] == "BDF")
				return state.get_args()["time"]["integrator"]["steps"].get<int>();
		...
		const int cur_step) // current time step
	{
		...
		const int dim = state.get_mesh().dimension();
		const int actual_dim = state.get_problem().is_scalar() ? 1 : dim;
		const int n_elements = int(bases.size());
		const double t0 = state.get_problem().is_time_dependent() ? state.get_args()["time"]["t0"].get<double>() : 0.0;
		const double dt = state.get_problem().is_time_dependent() ? state.get_args()["time"]["dt"].get<double>() : 0.0;
		...
		if (spatial_integral_type == SpatialIntegralType::Volume)
		{
			...
			utils::maybe_parallel_for(n_elements, [&](int start, int end, int thread_id) {
				...
				{
					if (interested_ids.size() != 0 && interested_ids.find(state.get_mesh().get_body_id(e)) == interested_ids.end())
						continue;

					assembler::ElementAssemblyValues &vals = local_storage.vals;
					state.assembly_cache().compute(e, state.get_mesh().is_volume(), bases[e], gbases[e], vals);
					io::Evaluator::interpolate_at_local_vals(e, dim, actual_dim, vals, solution, u, grad_u);

					const quadrature::Quadrature &quadrature = vals.quadrature;
					local_storage.da = vals.det.array() * quadrature.weights.array();

					const Eigen::MatrixXd lame_params = extract_lame_params(state.primary_assembler().parameters(), e, params.t, quadrature.points, vals.val);

					params.elem = e;
					params.body_id = state.get_mesh().get_body_id(e);
		...
		{
			auto storage = utils::create_thread_storage(LocalThreadScalarStorage());
			utils::maybe_parallel_for(state.boundary_state().total_local_boundary.size(), [&](int start, int end, int thread_id) {
				LocalThreadScalarStorage &local_storage = utils::get_local_thread_storage(storage, thread_id);
				...
				{
					const auto &lb = state.boundary_state().total_local_boundary[lb_id];
					const int e = lb.element_id();

					for (int i = 0; i < lb.size(); i++)
					{
						const int global_primitive_id = lb.global_primitive_id(i);
						if (interested_ids.size() != 0 && interested_ids.find(state.get_mesh().get_boundary_id(global_primitive_id)) == interested_ids.end())
							continue;

						utils::BoundarySampler::boundary_quadrature(lb, state.n_boundary_samples(), state.get_mesh(), i, false, uv, points, normal, weights);

						assembler::ElementAssemblyValues &vals = local_storage.vals;
						vals.compute(e, state.get_mesh().is_volume(), points, bases[e], gbases[e]);
						io::Evaluator::interpolate_at_local_vals(e, dim, actual_dim, vals, solution, u, grad_u);

						const Eigen::MatrixXd lame_params = extract_lame_params(state.primary_assembler().parameters(), e, params.t, points, vals.val);

						params.elem = e;
						params.body_id = state.get_mesh().get_body_id(e);
						params.boundary_id = state.get_mesh().get_boundary_id(global_primitive_id);
		...
		{
			std::vector<bool> traversed(state.primary_space().n_bases, false);
			IntegrableFunctional::ParameterType params;
			params.t = dt * cur_step + t0;
			params.step = cur_step;
			for (int e = 0; e < bases.size(); e++)
			{
				...
				for (int i = 0; i < bs.bases.size(); i++)
				{
					...
					const Eigen::MatrixXd lame_params = extract_lame_params(state.primary_assembler().parameters(), e, params.t, Eigen::MatrixXd::Zero(1, dim) /*Not used*/, g.node);
		...
		const int cur_time_step)
	{
		...
		const int n_elements = int(bases.size());
		term.setZero(state.primary_space().geometry->n_bases * dim, 1);
		...
		if (spatial_integral_type == SpatialIntegralType::Volume)
		{
			utils::maybe_parallel_for(n_elements, [&](int start, int end, int thread_id) {
				...
				for (int e = start; e < end; ++e)
				{
					...
					gvals.compute(e, state.get_mesh().is_volume(), vals.quadrature.points, gbases[e], gbases[e]);
			...
			utils::maybe_parallel_for(state.boundary_state().total_local_boundary.size(), [&](int start, int end, int thread_id) {
				...
				for (int lb_id = start; lb_id < end; ++lb_id)
				{
					...
					for (int i = 0; i < lb.size(); i++)
					{
						...
						assembler::ElementAssemblyValues &vals = local_storage.vals;
						io::Evaluator::interpolate_at_local_vals(state.get_mesh(), state.get_problem().is_scalar(), bases, gbases, e, points, solution, u, grad_u);
						// io::Evaluator::interpolate_at_local_vals(state.get_mesh(), state.get_problem().is_scalar(), gbases, gbases, e, points, global_positions, x, grad_x);

						vals.compute(e, state.get_mesh().is_volume(), points, gbases[e], gbases[e]);
						...
						const auto nodes = gbases[e].local_nodes_for_primitive(lb.global_primitive_id(i), state.get_mesh());
		...
		Eigen::VectorXd &one_form)
	{
		...
		one_form.setZero(state.primary_space().geometry->n_bases * state.get_mesh().dimension());
		Eigen::MatrixXd adjoint_zeroed = adjoint;
		adjoint_zeroed(state.boundary_state().boundary_nodes, Eigen::all).setZero();

		// if (j.depend_on_u() || j.depend_on_gradu())
		{
			ElasticForceDerivative::force_shape_derivative(*state.solve_data()->elastic_form, 0, state.primary_space().geometry->n_bases, sol, sol, adjoint_zeroed, elasticity_term);
			if (state.solve_data()->body_form)
				BodyForceDerivative::force_shape_derivative(*state.solve_data()->body_form, state.primary_space().geometry->n_bases, 0, sol, adjoint_zeroed, rhs_term);
			else
				rhs_term.setZero(one_form.size());

			if (state.solve_data()->pressure_form)
			{
				PressureForceDerivative::force_shape_derivative(*state.solve_data()->pressure_form, state.primary_space().geometry->n_bases, 0, sol, adjoint_zeroed, pressure_term);
		...
		one_form = utils::flatten(utils::unflatten(one_form, state.get_mesh().dimension())(state.primitive_to_node(), Eigen::all));
	}
		...
		Eigen::VectorXd &one_form)
	{
		...
		const int dim = state.get_mesh().dimension();
		one_form.setZero(state.primary_space().geometry->n_bases * dim);

		const Eigen::MatrixXd affine_adjoint = homo_problem->reduced_to_disp_grad(adjoint, true);
		const Eigen::VectorXd full_adjoint = homo_problem->NLProblem::reduced_to_full(adjoint.topRows(homo_problem->reduced_size())) + io::Evaluator::generate_linear_field(state.primary_space().n_bases, state.primary_space().mesh_nodes, affine_adjoint);

		ElasticForceDerivative::force_shape_derivative(*state.solve_data()->elastic_form, 0, state.primary_space().geometry->n_bases, sol, sol, full_adjoint, elasticity_term);
	...
	void AdjointTools::dJ_shape_transient_adjoint_term(
		...
		Eigen::VectorXd &one_form)
	{
		const double t0 = state.get_args()["time"]["t0"];
		const double dt = state.get_args()["time"]["dt"];
		const int time_steps = state.get_args()["time"]["time_steps"];
		...
		for (int i = time_steps; i > 0; --i)
		{
			...
			cur_p(state.boundary_state().boundary_nodes).setZero();
			cur_nu(state.boundary_state().boundary_nodes).setZero();

			{
				InertiaForceDerivative::force_shape_derivative(*state.solve_data()->inertia_form, state.get_mesh().is_volume(), state.primary_space().geometry->n_bases, t, state.primary_space().basis_list(), state.primary_space().geometry_basis_list(), state.mass_assembler(), state.mass_assembly_cache(), velocity, cur_nu, mass_term);
				ElasticForceDerivative::force_shape_derivative(*state.solve_data()->elastic_form, t, state.primary_space().geometry->n_bases, diff_cache.u(i), diff_cache.u(i), cur_p, elasticity_term);
				BodyForceDerivative::force_shape_derivative(*state.solve_data()->body_form, state.primary_space().geometry->n_bases, t, diff_cache.u(i - 1), cur_p, rhs_term);
				PressureForceDerivative::force_shape_derivative(*state.solve_data()->pressure_form, state.primary_space().geometry->n_bases, t, diff_cache.u(i), cur_p, pressure_term);
				pressure_term = diff_cache.basis_nodes_to_gbasis_nodes() * pressure_term;

				if (state.solve_data()->damping_form)
					ElasticForceDerivative::force_shape_derivative(*state.solve_data()->damping_form, t, state.primary_space().geometry->n_bases, diff_cache.u(i), diff_cache.u(i - 1), cur_p, damping_term);
				else
		...
		sum_alpha_p(state.boundary_state().boundary_nodes).setZero();
		InertiaForceDerivative::force_shape_derivative(*state.solve_data()->inertia_form, state.get_mesh().is_volume(), state.primary_space().geometry->n_bases, t0, state.primary_space().basis_list(), state.primary_space().geometry_basis_list(), state.mass_assembler(), state.mass_assembly_cache(), diff_cache.v(0), sum_alpha_p, mass_term);
		...
		Eigen::VectorXd &one_form)
	{
		...
		one_form.setZero(state.primary_space().basis_list().size() * 2);
		...
		utils::maybe_parallel_for(time_steps, [&](int start, int end, int thread_id) {
			...
			for (int i_aux = start; i_aux < end; ++i_aux)
			{
				...
				ElasticForceDerivative::force_material_derivative(*state.solve_data()->elastic_form, t0 + dt * i, diff_cache.u(i), diff_cache.u(i - 1), -cur_p, elasticity_term);
		...
		Eigen::VectorXd &one_form)
	{
		...
		std::shared_ptr<time_integrator::ImplicitTimeIntegrator> time_integrator =
			time_integrator::ImplicitTimeIntegrator::construct_time_integrator(state.get_args()["time"]["integrator"]);
		{
		...
		for (int t = 1; t <= time_steps; ++t)
		{
			...
			const Eigen::MatrixXd surface_solution_prev = state.collision_mesh().vertices(utils::unflatten(diff_cache.u(t - 1), dim));
			// const Eigen::MatrixXd surface_solution = state.collision_mesh().vertices(utils::unflatten(diff_cache.u(t), dim));

			const Eigen::MatrixXd surface_velocities = state.collision_mesh().map_displacements(utils::unflatten(time_integrator->compute_velocity(diff_cache.u(t)), state.collision_mesh().dim()));
		...
		Eigen::VectorXd &one_form)
	{
		...
		utils::maybe_parallel_for(time_steps, [&](int start, int end, int thread_id) {
			...
			for (int t_aux = start; t_aux < end; ++t_aux)
			{
				...
				ElasticForceDerivative::force_material_derivative(*state.solve_data()->damping_form, t * dt + t0, diff_cache.u(t), diff_cache.u(t - 1), -cur_p, damping_term);
	...
	void AdjointTools::dJ_initial_condition_adjoint_term(
		...
		Eigen::VectorXd &one_form)
	{
		const int ndof = state.primary_space().ndof();
		...
		for (int b : state.boundary_state().boundary_nodes)
		{
			one_form(b) = 0;
			one_form(ndof + b) = 0;
		}
	...
	void AdjointTools::dJ_dirichlet_static_adjoint_term(
		...
		Eigen::VectorXd &one_form)
	{
		const int n_dirichlet_dof = state.boundary_state().boundary_nodes.size();
		StiffnessMatrix gradd_h = diff_cache.gradu_h(0);
		std::set<int> boundary_nodes_set(state.boundary_state().boundary_nodes.begin(), state.boundary_state().boundary_nodes.end());
		...
		one_form.setZero(state.primary_space().ndof());
		one_form(state.boundary_state().boundary_nodes) -= adjoint(state.boundary_state().boundary_nodes, 0);
		one_form(state.boundary_state().boundary_nodes) -= (gradd_h.transpose() * adjoint.col(0))(state.boundary_state().boundary_nodes);
		one_form = utils::flatten(utils::unflatten(one_form, state.get_mesh().dimension())(state.primitive_to_node(), Eigen::all));
	}
		...
		Eigen::VectorXd &one_form)
	{
		...
		for (int i = 1; i <= time_steps; ++i)
		{
			...
			one_form.segment((i - 1) * n_dirichlet_dof, n_dirichlet_dof) = -(1. / beta_dt) * adjoint_p(state.boundary_state().boundary_nodes, i);
		}
	}
		...
		for (int i = 0; i < boundary_ids.size(); ++i)
		{
			double pressure_term = PressureForceDerivative::force_pressure_derivative(
				*state.solve_data()->pressure_form,
				state.primary_space().geometry->n_bases,
				0,
				boundary_ids[i],
				sol,
				adjoint);
		...
		Eigen::VectorXd &term)
	{
		...
		term = Eigen::MatrixXd::Zero(state.primary_space().n_bases * actual_dim, 1);
			...
			utils::maybe_parallel_for(state.boundary_state().total_local_boundary.size(), [&](int start, int end, int thread_id) {
				...
				for (int lb_id = start; lb_id < end; ++lb_id)
				{
					...
					for (int i = 0; i < lb.size(); i++)
					{
						...
						{
							const auto nodes = bases[e].local_nodes_for_primitive(lb.global_primitive_id(l), state.get_mesh());
	...
	Eigen::VectorXd AdjointTools::map_primitive_to_node_order(const varform::VarForm &state, const Eigen::VectorXd &primitives)
	{
		int dim = state.get_mesh().dimension();
		assert(primitives.size() == (state.primary_space().geometry->n_bases * dim));
		Eigen::VectorXd nodes(primitives.size());
		auto map = state.primitive_to_node();
		for (int v = 0; v < state.primary_space().geometry->n_bases; ++v)
			nodes.segment(map[v] * dim, dim) = primitives.segment(v * dim, dim);
		return nodes;
	}

	Eigen::VectorXd AdjointTools::map_node_to_primitive_order(const varform::VarForm &state, const Eigen::VectorXd &nodes)
	{
		int dim = state.get_mesh().dimension();
		assert(nodes.size() == (state.primary_space().geometry->n_bases * dim));
