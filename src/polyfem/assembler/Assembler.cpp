...
namespace polyfem::assembler
{
		...
		try
		{
			...
			maybe_parallel_for(n_bases, [&](int start, int end, int thread_id) {
				...
				for (int e = start; e < end; ++e)
				{
					...
					{
						local_storage.da(q) *= density(quadrature.points.row(q), vals.val.row(q), vals.element_id);
					}
					const int n_loc_bases = int(vals.basis_values.size());
		...
		StiffnessMatrix &stiffness) const
	{
		...
		maybe_parallel_for(n_bases, [&](int start, int end, int thread_id) {
			...
			for (int e = start; e < end; ++e)
			{
				...
				for (int q = 0; q < local_storage.da.size(); q++)
				{
					local_storage.da(q) *= density(quadrature.points.row(q), phi_vals.val.row(q), phi_vals.element_id);
				}
				const int n_phi_loc_bases = int(phi_vals.basis_values.size());
				const int n_psi_loc_bases = int(psi_vals.basis_values.size());
	...
	template class NLAssembler<GenericElastic<AMIPSEnergy>>;
