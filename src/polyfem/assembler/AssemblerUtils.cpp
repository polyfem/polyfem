...
namespace polyfem
{
	...
	namespace assembler
	{
		...
		static const std::unordered_map<std::string, FormulationProperties> formulation_properties = {
			...
			{"Multiscale",                     {/*is_scalar=*/false, /*is_fluid=*/false, /*is_mixed=*/false, /*is_solution_displacement=*/true,  /*is_linear=*/false}},
											   ...
											   bool serial) const
		{
			if (assembler == "SaintVenant")
				...
				return multiscale_.assemble(is_volume, bases, gbases, cache, dt, displacement, displacement_prev, serial);
													  ...
													  Eigen::MatrixXd &grad) const
		{
			if (assembler == "SaintVenant")
				...
				multiscale_.assemble_grad(is_volume, n_basis, bases, gbases, cache, dt, displacement, displacement_prev, grad);
													  ...
													  Eigen::MatrixXd &grad) const
		{
			if (assembler == "SaintVenant")
				...
				multiscale_.assemble_grad(is_volume, n_basis, bases, gbases, cache, dt, displacement, displacement_prev, projection, grad);
			else if (assembler == "DampingPrev")
													 ...
													 StiffnessMatrix &hessian) const
		{
			if (assembler == "SaintVenant")
				...
				multiscale_.assemble_hessian(is_volume, n_basis, project_to_psd, bases, gbases, cache, dt, displacement, displacement_prev, mat_cache, hessian);
			else if (assembler == "DampingPrev")
													 ...
													 Eigen::MatrixXd &hessian) const
		{
			if (assembler == "SaintVenant")
				...
				multiscale_.assemble_hessian(is_volume, n_basis, project_to_psd, bases, gbases, cache, dt, displacement, displacement_prev, projection, hessian);
			else if (assembler == "DampingPrev")
												  ...
												  Eigen::MatrixXd &result) const
		{
			if (assembler == "Mass" || assembler == "Laplacian" || assembler == "Helmholtz" || assembler == "Bilaplacian")
				...
				multiscale_.local_assembler().compute_von_mises_stresses(el_id, bs, gbs, local_pts, fun, result);
												  ...
												  Eigen::MatrixXd &result) const
		{
			if (assembler == "Mass" || assembler == "Laplacian" || assembler == "Helmholtz" || assembler == "Bilaplacian")
				...
				multiscale_.local_assembler().compute_stress_tensor(el_id, bs, gbs, local_pts, fun, result);
		...
		void AssemblerUtils::update_lame_params(const LameParameters &newParams)
		{
			...
			multiscale_.local_assembler().set_params(newParams);
		...
		void AssemblerUtils::update_lame_params(const Eigen::MatrixXd &lambdas, const Eigen::MatrixXd &mus)
		{
			...
			multiscale_.local_assembler().lame_params().lambda_mat_ = lambdas;
