...
namespace polyfem::assembler
{
	...
	void Electrostatics::add_multimaterial(const int index, const json &params, const Units &units, const io::ResourceIO &resources)
	{
		assert(size() == 1);

		epsilon_.add_multimaterial(index, params, units.permittivity(), resources);
	}
