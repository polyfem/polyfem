...
namespace polyfem
{
	...
	namespace assembler
	{
		...
		void RhsAssembler::assemble_homogenization(Eigen::MatrixXd &rhs) const
		{
			...
			rhs = Eigen::MatrixXd::Zero(n_basis_ * size_, neq);
			if (formulation_ == "LinearElasticity")
			{
			...
			else if (formulation_ == "Stokes")
			{
				...
				for (int e = 0; e < n_elements; ++e)
				{
					...
					{
						const AssemblyValues &v = vals.basis_values[i];
						const double rhs_value = (vals.det.array() * quadrature.weights.array() * v.val.array()).sum();

						for (int d = 0; d < size_; ++d)
							for (std::size_t ii = 0; ii < v.global.size(); ++ii)
								rhs(v.global[ii].index * size_ + d, d) += rhs_value * v.global[ii].val;
					}
				}
			}
		}
