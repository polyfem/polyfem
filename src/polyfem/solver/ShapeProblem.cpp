...
namespace polyfem
{
    ...
    ShapeProblem::ShapeProblem(State &state_, const std::shared_ptr<CompositeFunctional> j_, const json &args) : OptimizationProblem(state_, j_, args)
    {
		...
		x_to_param = [this](const TVector& x, Eigen::MatrixXd& V)
		{
    ...
    double ShapeProblem::barrier_energy(const TVector &x)
    {
        ...
        return _barrier_stiffness * ipc::compute_barrier_potential(
            state.collision_mesh, state.collision_mesh.vertices(V), _constraint_set, _dhat);
    }
