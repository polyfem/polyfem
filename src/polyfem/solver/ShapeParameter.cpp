...
namespace polyfem
{
	...
	ShapeParameter::ShapeParameter(std::vector<std::shared_ptr<State>> states_ptr) : Parameter(states_ptr)
	{
		...
		param_to_x = [this](Eigen::VectorXd &x, const Eigen::MatrixXd &V) {
			x.setZero(V.rows() * this->dim, 1);
			for (int i = 0; i < V.rows(); i++)
				for (int d = 0; d < this->dim; d++)
					x(i * this->dim + d) = V(i, d);
		};
		dparam_to_dx = [this](Eigen::VectorXd &grad_x, const Eigen::VectorXd &grad_v) {
			grad_x = grad_v;
	...
	void ShapeParameter::build_fixed_nodes()
	{
		...
		V.conservativeResize(V.rows(), states_ptr_[0]->mesh->dimension());
