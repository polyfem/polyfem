...
namespace polyfem::io
{
		...
		const bool is_contact_enabled) const
	{
		...
		if (is_contact_enabled && (opts.contact_forces || opts.friction_forces || opts.normal_adhesion_forces || opts.tangential_adhesion_forces || opts.export_field("gcp_candidates") || opts.export_field("gcp_normals")))
		{
		...
		const bool is_contact_enabled) const
	{
		...
		// Export GCP collision mesh normals (2D only) using exactly the same
		// mesh.edges() adjacency data that Point2 and Edge2 use internally.
		//
		// gcp_edge_normals  : per-edge active-side normal (cell field).
		//   Edge2 activates when cross2(d, v1-v0) > 0, i.e. d is to the RIGHT
		//   of the stored tangent.  So the active-side normal = right-normal of
		//   the stored tangent = (t.y, -t.x).
		//
		// gcp_vertex_normals: per-vertex inward normal (point field).
		//   Point2 uses t0 = (neighbors[0] - v).normalized() where neighbors[0]
		//   is from edges where v is at position 0 (outgoing edge).
		//   Inward normal = left-normal of t0 = (-t0.y, t0.x).
		if (contact_form && state.args["contact"]["use_gcp_formulation"]
		    && problem_dim == 2 && opts.export_field("gcp_normals"))
		{
			const int nv = collision_mesh.num_vertices();
			const int ne = collision_mesh.num_edges();

			// --- per-edge normals: right-normal of stored tangent (Edge2 active-side) ---
			Eigen::MatrixXd edge_normals(ne, 2);
			for (int e = 0; e < ne; ++e)
			{
				const int v0 = collision_mesh.edges()(e, 0);
				const int v1 = collision_mesh.edges()(e, 1);
				Eigen::Vector2d t =
				    displaced_surface.row(v1).head<2>()
				    - displaced_surface.row(v0).head<2>();
				const double len = t.norm();
				if (len > 1e-14) t /= len;
				edge_normals.row(e) = Eigen::Vector2d(t.y(), -t.x());
			}
			writer.add_cell_field("gcp_edge_normals", edge_normals);

			// --- per-vertex normals: left-normal of outgoing tangent t0 (Point2) ---
			// Uses point2_find_adjacent_vertices — same function as Point2 constructor.
			Eigen::MatrixXd vert_normals(nv, 2);
			vert_normals.setZero();
			for (int vi = 0; vi < nv; ++vi)
			{
				const auto nbrs = ipc::point2_find_adjacent_vertices(collision_mesh, vi);
				if (nbrs[0] < 0) continue; // no outgoing edge
				const Eigen::Vector2d t0 =
				    (displaced_surface.row(nbrs[0]).head<2>()
				     - displaced_surface.row(vi).head<2>()).normalized();
				// left-normal of t0 = inward normal used by Point2
				vert_normals.row(vi) = Eigen::Vector2d(-t0.y(), t0.x());
			}
			writer.add_field("gcp_vertex_normals", vert_normals);
		}

		// Export GCP collision candidates as an expanded point-cloud VTU.
		// Each candidate contributes all of its stencil vertices (not a single anchor point),
		// with per-point metadata linking each point back to its source candidate.
		if (contact_form && state.args["contact"]["use_gcp_formulation"] && opts.export_field("gcp_candidates"))
		{
			const auto form = std::dynamic_pointer_cast<solver::SmoothContactForm>(contact_form);
			assert(form);
			const auto &gcp_set = form->collision_set();
			const int n_candidates = static_cast<int>(gcp_set.size());

			auto gather_active_vertices = [&](const auto &col) {
				std::array<int, 4> vids = { -1, -1, -1, -1 };
				switch (col.type())
				{
				case ipc::CollisionType::VERTEX_VERTEX:
					vids[0] = int(col[0]);
					vids[1] = int(col[1]);
					break;
				case ipc::CollisionType::EDGE_VERTEX:
				{
					const int e = int(col[0]);
					vids[0] = collision_mesh.edges()(e, 0);
					vids[1] = collision_mesh.edges()(e, 1);
					vids[2] = int(col[1]);
					break;
				}
				case ipc::CollisionType::EDGE_EDGE:
				{
					const int ea = int(col[0]);
					const int eb = int(col[1]);
					vids[0] = collision_mesh.edges()(ea, 0);
					vids[1] = collision_mesh.edges()(ea, 1);
					vids[2] = collision_mesh.edges()(eb, 0);
					vids[3] = collision_mesh.edges()(eb, 1);
					break;
				}
				case ipc::CollisionType::FACE_VERTEX:
				{
					const int f = int(col[0]);
					vids[0] = collision_mesh.faces()(f, 0);
					vids[1] = collision_mesh.faces()(f, 1);
					vids[2] = collision_mesh.faces()(f, 2);
					vids[3] = int(col[1]);
					break;
				}
				default:
				{
					const auto v = col.vertex_ids();
					for (int k = 0; k < 4; ++k)
						vids[k] = int(v[k]);
					break;
				}
			...
			{
				const auto vids = gather_active_vertices(gcp_set[ci]);
				for (int k = 0; k < 4; ++k)
					n_points += vids[k] >= 0 ? 1 : 0;
			}

			logger().info(
				"GCP candidates: {} total, {} n_candidates, {} exported stencil points",
				n_candidates, gcp_set.n_candidates(), n_points);

			Eigen::MatrixXd cand_pts(n_points, problem_dim);
			Eigen::VectorXd cand_dist(n_points), cand_potential(n_points), cand_active(n_points), cand_type(n_points);
			Eigen::VectorXd cand_candidate_id(n_points), cand_candidate_size(n_points), cand_slot(n_points), cand_vertex_id(n_points);
			Eigen::VectorXd cand_prim0(n_points), cand_prim1(n_points);
			Eigen::MatrixXd pair_pts(2 * n_candidates, problem_dim);
			Eigen::MatrixXi pair_cells(n_candidates, 2);
			Eigen::VectorXd pair_candidate_id(n_candidates), pair_type(n_candidates), pair_dist(n_candidates), pair_potential(n_candidates), pair_active(n_candidates);
			Eigen::VectorXd pair_prim0(n_candidates), pair_prim1(n_candidates);
			Eigen::VectorXd pair_active_match(n_candidates), pair_active_ev_count_at_vertex(n_candidates), pair_active_ev_count_at_edge(n_candidates), pair_active_vv_count_at_vertex(n_candidates);
			std::vector<Eigen::Vector2i> cand_line_cells;
			std::vector<double> cand_line_candidate_id, cand_line_type, cand_line_kind;
			std::vector<double> cand_line_slot_a, cand_line_slot_b;
			std::vector<double> cand_line_vertex_a, cand_line_vertex_b;
			int n_vv = 0, n_ev = 0, n_ee = 0, n_fv = 0, n_other = 0;
			int pi = 0;

			for (int ci = 0; ci < n_candidates; ++ci)
			{
				const auto &col = gcp_set[ci];
				const auto vids = gather_active_vertices(col);
				const double dist = std::sqrt(std::max(0.0, col.compute_distance(displaced_surface)));
				const double potential = col(col.dof(displaced_surface), form->get_params());
				const double is_active = col.is_active() ? 1.0 : 0.0;
				const int prim0 = int(col[0]);
				const int prim1 = int(col[1]);
				std::array<int, 4> point_idx_by_slot = { -1, -1, -1, -1 };
				Eigen::RowVectorXd pair_a = Eigen::RowVectorXd::Zero(problem_dim);
				Eigen::RowVectorXd pair_b = Eigen::RowVectorXd::Zero(problem_dim);
				...
				case ipc::CollisionType::VERTEX_VERTEX:
					n_vv++;
					type_code = 1;
					pair_a = displaced_surface.row(col[0]).head(problem_dim);
					pair_b = displaced_surface.row(col[1]).head(problem_dim);
					break;
				case ipc::CollisionType::EDGE_VERTEX:
				{
					n_ev++;
					type_code = 0;
					const int e = int(col[0]);
					const int v = int(col[1]);
					pair_a = displaced_surface.row(v).head(problem_dim);
					pair_b = 0.5 * (
						displaced_surface.row(collision_mesh.edges()(e, 0)).head(problem_dim)
						+ displaced_surface.row(collision_mesh.edges()(e, 1)).head(problem_dim));
					break;
				}
				case ipc::CollisionType::EDGE_EDGE:
				{
					...
					const int eb = int(col[1]);
					pair_a = 0.5 * (
						displaced_surface.row(collision_mesh.edges()(ea, 0)).head(problem_dim)
						+ displaced_surface.row(collision_mesh.edges()(ea, 1)).head(problem_dim));
					pair_b = 0.5 * (
						displaced_surface.row(collision_mesh.edges()(eb, 0)).head(problem_dim)
						+ displaced_surface.row(collision_mesh.edges()(eb, 1)).head(problem_dim));
					break;
				}
				case ipc::CollisionType::FACE_VERTEX:
				{
					...
					for (int lv = 0; lv < 3; ++lv)
						pair_b += displaced_surface.row(collision_mesh.faces()(f, lv)).head(problem_dim);
					pair_b /= 3.0;
					break;
				}
				default:
					...
					for (int k = 0; k < 4; ++k)
					{
						if (vids[k] >= 0)
						{
							pair_a = displaced_surface.row(vids[k]).head(problem_dim);
							break;
						}
					}
					for (int k = 1; k < 4; ++k)
					{
						if (vids[k] >= 0)
						{
							pair_b = displaced_surface.row(vids[k]).head(problem_dim);
							break;
						}
					}
					if ((pair_b - pair_a).norm() == 0 && vids[0] >= 0)
						pair_b = pair_a;
					break;
				}

				pair_pts.row(2 * ci) = pair_a;
				pair_pts.row(2 * ci + 1) = pair_b;
				pair_cells(ci, 0) = 2 * ci;
				pair_cells(ci, 1) = 2 * ci + 1;
				pair_candidate_id(ci) = ci;
				pair_type(ci) = type_code;
				pair_dist(ci) = dist;
				pair_potential(ci) = potential;
				pair_active(ci) = is_active;
				pair_prim0(ci) = prim0;
				pair_prim1(ci) = prim1;
				...
				for (int slot = 0; slot < 4; ++slot)
				{
					if (vids[slot] < 0)
						continue;
					point_idx_by_slot[slot] = pi;

					cand_pts.row(pi) = displaced_surface.row(vids[slot]).head(problem_dim);
					cand_dist(pi) = dist;
					cand_potential(pi) = potential;
					cand_active(pi) = is_active;
					cand_type(pi) = type_code;
					cand_candidate_id(pi) = ci;
					cand_candidate_size(pi) = csize;
					cand_slot(pi) = slot;
					cand_vertex_id(pi) = vids[slot];
					cand_prim0(pi) = prim0;
					cand_prim1(pi) = prim1;
					++pi;
				}

				auto add_line = [&](const int slot_a, const int slot_b, const double kind) {
					const int pa = point_idx_by_slot[slot_a];
					const int pb = point_idx_by_slot[slot_b];
					if (pa < 0 || pb < 0 || pa == pb)
						return;
					cand_line_cells.emplace_back(pa, pb);
					cand_line_candidate_id.push_back(ci);
					cand_line_type.push_back(type_code);
					cand_line_kind.push_back(kind);
					cand_line_slot_a.push_back(slot_a);
					cand_line_slot_b.push_back(slot_b);
					cand_line_vertex_a.push_back(vids[slot_a]);
					cand_line_vertex_b.push_back(vids[slot_b]);
				};
				...
				case 1: // VV
					...
					break;
				case 0: // EV
					add_line(0, 1, 0); // edge primitive
					add_line(2, 0, 1); // cross links
					add_line(2, 1, 1);
					break;
				case 2: // EE
					add_line(0, 1, 0); // edge 0
					add_line(2, 3, 1); // edge 1
					add_line(0, 2, 2); // cross links
					add_line(0, 3, 2);
					add_line(1, 2, 2);
					add_line(1, 3, 2);
					break;
				case 3: // FV
					add_line(0, 1, 0); // face edges
					add_line(1, 2, 0);
					add_line(2, 0, 0);
					add_line(3, 0, 1); // vertex-face links
					add_line(3, 1, 1);
					add_line(3, 2, 1);
					break;
				default:
			...
			// One VTK_VERTEX cell per exported stencil point.
			paraviewo::VTUWriter cand_writer;
			cand_writer.add_field("distance", cand_dist);
			cand_writer.add_field("potential", cand_potential);
			cand_writer.add_field("is_active", cand_active);
			cand_writer.add_field("type", cand_type);
			cand_writer.add_field("candidate_id", cand_candidate_id);
			cand_writer.add_field("candidate_size", cand_candidate_size);
			cand_writer.add_field("stencil_slot", cand_slot);
			cand_writer.add_field("vertex_id", cand_vertex_id);
			cand_writer.add_field("primitive0_id", cand_prim0);
			cand_writer.add_field("primitive1_id", cand_prim1);
			Eigen::MatrixXi cand_cells(n_points, 1);
			for (int pi = 0; pi < n_points; ++pi)
				cand_cells(pi, 0) = pi;
			cand_writer.write_mesh(
				export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates.vtu",
				cand_pts, cand_cells);

			paraviewo::VTUWriter pair_writer;
			pair_writer.add_cell_field("candidate_id", pair_candidate_id);
			pair_writer.add_cell_field("type", pair_type);
			pair_writer.add_cell_field("distance", pair_dist);
			pair_writer.add_cell_field("potential", pair_potential);
			pair_writer.add_cell_field("is_active", pair_active);
			pair_writer.add_cell_field("primitive0_id", pair_prim0);
			pair_writer.add_cell_field("primitive1_id", pair_prim1);
			pair_writer.write_mesh(
				export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates_pairs.vtu",
				pair_pts, pair_cells);
			logger().info("GCP active candidate pairs export: {}", n_candidates);

			auto write_active_pair_subset = [&](const int type_code, const std::string &suffix) {
				std::vector<int> selected;
				selected.reserve(n_candidates);
				for (int ci = 0; ci < n_candidates; ++ci)
				{
					if (int(pair_type(ci)) == type_code)
				...
				const int m = int(selected.size());
				Eigen::MatrixXd sub_pts(2 * m, problem_dim);
				Eigen::MatrixXi sub_cells(m, 2);
				Eigen::VectorXd sub_candidate_id(m), sub_type(m), sub_dist(m), sub_potential(m), sub_active(m), sub_prim0(m), sub_prim1(m);

				for (int j = 0; j < m; ++j)
				{
					const int ci = selected[j];
					sub_pts.row(2 * j) = pair_pts.row(2 * ci);
					sub_pts.row(2 * j + 1) = pair_pts.row(2 * ci + 1);
					sub_cells(j, 0) = 2 * j;
					sub_cells(j, 1) = 2 * j + 1;
					sub_candidate_id(j) = pair_candidate_id(ci);
					sub_type(j) = pair_type(ci);
					sub_dist(j) = pair_dist(ci);
					sub_potential(j) = pair_potential(ci);
					sub_active(j) = pair_active(ci);
					sub_prim0(j) = pair_prim0(ci);
					sub_prim1(j) = pair_prim1(ci);
				}

				paraviewo::VTUWriter sub_writer;
				sub_writer.add_cell_field("candidate_id", sub_candidate_id);
				sub_writer.add_cell_field("type", sub_type);
				sub_writer.add_cell_field("distance", sub_dist);
				sub_writer.add_cell_field("potential", sub_potential);
				sub_writer.add_cell_field("is_active", sub_active);
				sub_writer.add_cell_field("primitive0_id", sub_prim0);
				sub_writer.add_cell_field("primitive1_id", sub_prim1);
				sub_writer.write_mesh(
					export_surface.substr(0, export_surface.length() - 4) + suffix,
					sub_pts, sub_cells);
				logger().info("GCP active candidate pairs {} export: {}", suffix, m);
			};

			write_active_pair_subset(1, "_gcp_candidates_pairs_vv.vtu");
			write_active_pair_subset(0, "_gcp_candidates_pairs_ev.vtu");

			if (!cand_line_cells.empty())
			{
				const int n_lines = int(cand_line_cells.size());
				Eigen::MatrixXi line_cells(n_lines, 2);
				for (int li = 0; li < n_lines; ++li)
					line_cells.row(li) = cand_line_cells[li];

				Eigen::VectorXd line_candidate_id(n_lines), line_type(n_lines), line_kind(n_lines);
				Eigen::VectorXd line_slot_a(n_lines), line_slot_b(n_lines), line_vertex_a(n_lines), line_vertex_b(n_lines);
				for (int li = 0; li < n_lines; ++li)
				{
					line_candidate_id(li) = cand_line_candidate_id[li];
					line_type(li) = cand_line_type[li];
					line_kind(li) = cand_line_kind[li];
					line_slot_a(li) = cand_line_slot_a[li];
					line_slot_b(li) = cand_line_slot_b[li];
					line_vertex_a(li) = cand_line_vertex_a[li];
					line_vertex_b(li) = cand_line_vertex_b[li];
				}

				paraviewo::VTUWriter cand_line_writer;
				cand_line_writer.add_field("distance", cand_dist);
				cand_line_writer.add_field("potential", cand_potential);
				cand_line_writer.add_field("is_active", cand_active);
				cand_line_writer.add_field("type", cand_type);
				cand_line_writer.add_field("candidate_id", cand_candidate_id);
				cand_line_writer.add_field("candidate_size", cand_candidate_size);
				cand_line_writer.add_field("stencil_slot", cand_slot);
				cand_line_writer.add_field("vertex_id", cand_vertex_id);
				cand_line_writer.add_field("primitive0_id", cand_prim0);
				cand_line_writer.add_field("primitive1_id", cand_prim1);
				cand_line_writer.add_cell_field("line_candidate_id", line_candidate_id);
				cand_line_writer.add_cell_field("line_type", line_type);
				cand_line_writer.add_cell_field("line_kind", line_kind);
				cand_line_writer.add_cell_field("line_slot_a", line_slot_a);
				cand_line_writer.add_cell_field("line_slot_b", line_slot_b);
				cand_line_writer.add_cell_field("line_vertex_a", line_vertex_a);
				cand_line_writer.add_cell_field("line_vertex_b", line_vertex_b);
				cand_line_writer.write_mesh(
					export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates_lines.vtu",
					cand_pts, line_cells);
				logger().info("GCP active candidate lines export: {}", n_lines);
			}
			logger().info(
				"GCP active types: vv={} ev={} ee={} fv={} other={}",
				n_vv, n_ev, n_ee, n_fv, n_other);
		}

		// Export full broad-phase GCP candidates as expanded stencil points.
		if (contact_form && state.args["contact"]["use_gcp_formulation"] && opts.export_field("gcp_candidates_full"))
		{
			const auto form = std::dynamic_pointer_cast<solver::SmoothContactForm>(contact_form);
			assert(form);
			const auto &gcp_set = form->collision_set();
			const auto &full_set = gcp_set.m_candidates;
			const int n_candidates = static_cast<int>(full_set.size());

			auto gather_full_vertices = [&](const auto &cand, int &prim0, int &prim1, double &type_code) {
				std::array<int, 4> vids = { -1, -1, -1, -1 };
				prim0 = -1;
				prim1 = -1;
				type_code = -1;

				if (const auto *ev = dynamic_cast<const ipc::EdgeVertexCandidate *>(&cand))
				{
					prim0 = int(ev->edge_id);
					prim1 = int(ev->vertex_id);
					type_code = 0;
					const int e = int(ev->edge_id);
					vids[0] = collision_mesh.edges()(e, 0);
					vids[1] = collision_mesh.edges()(e, 1);
					vids[2] = int(ev->vertex_id);
				}
				else if (const auto *vv = dynamic_cast<const ipc::VertexVertexCandidate *>(&cand))
				{
					prim0 = int(vv->vertex0_id);
					prim1 = int(vv->vertex1_id);
					type_code = 1;
					vids[0] = int(vv->vertex0_id);
					vids[1] = int(vv->vertex1_id);
				}
				else if (const auto *ee = dynamic_cast<const ipc::EdgeEdgeCandidate *>(&cand))
				{
					prim0 = int(ee->edge0_id);
					prim1 = int(ee->edge1_id);
					type_code = 2;
					const int ea = int(ee->edge0_id);
					const int eb = int(ee->edge1_id);
					vids[0] = collision_mesh.edges()(ea, 0);
					vids[1] = collision_mesh.edges()(ea, 1);
					vids[2] = collision_mesh.edges()(eb, 0);
					vids[3] = collision_mesh.edges()(eb, 1);
				}
				else if (const auto *fv = dynamic_cast<const ipc::FaceVertexCandidate *>(&cand))
				{
					prim0 = int(fv->face_id);
					prim1 = int(fv->vertex_id);
					type_code = 3;
					const int f = int(fv->face_id);
					vids[0] = collision_mesh.faces()(f, 0);
					vids[1] = collision_mesh.faces()(f, 1);
					vids[2] = collision_mesh.faces()(f, 2);
					vids[3] = int(fv->vertex_id);
				}
				else
				{
					const auto v = cand.vertex_ids(collision_mesh.edges(), collision_mesh.faces());
					for (int k = 0; k < 4; ++k)
						vids[k] = int(v[k]);
				}
			...
			for (int ci = 0; ci < n_candidates; ++ci)
			{
				int prim0, prim1;
				double type_code;
				const auto vids = gather_full_vertices(full_set[ci], prim0, prim1, type_code);
				for (int k = 0; k < 4; ++k)
					n_points += vids[k] >= 0 ? 1 : 0;
			}

			logger().info("GCP full candidates export: {} candidates, {} stencil points", n_candidates, n_points);
			...
			std::vector<double> cand_line_slot_a, cand_line_slot_b;
			std::vector<double> cand_line_vertex_a, cand_line_vertex_b;
			int n_full_vv = 0, n_full_ev = 0, n_full_ee = 0, n_full_fv = 0, n_full_other = 0;
			int pi = 0;

			std::unordered_set<long long> active_ev_pairs;
			std::unordered_map<int, int> active_ev_count_by_vertex;
			std::unordered_map<int, int> active_ev_count_by_edge;
			std::unordered_map<int, int> active_vv_count_by_vertex;
			for (int ai = 0; ai < int(gcp_set.size()); ++ai)
			{
				const auto &acol = gcp_set[ai];
				switch (acol.type())
				{
				case ipc::CollisionType::EDGE_VERTEX:
				{
					const int e = int(acol[0]);
					const int v = int(acol[1]);
					const long long key = (static_cast<long long>(e) << 32) | static_cast<unsigned int>(v);
					active_ev_pairs.insert(key);
					active_ev_count_by_vertex[v]++;
					active_ev_count_by_edge[e]++;
					break;
				}
				case ipc::CollisionType::VERTEX_VERTEX:
				{
					const int v0 = int(acol[0]);
					const int v1 = int(acol[1]);
					active_vv_count_by_vertex[v0]++;
					active_vv_count_by_vertex[v1]++;
					break;
				}
				default:
					break;
			...
			for (int ci = 0; ci < n_candidates; ++ci)
			{
				const auto &cand = full_set[ci];

				int prim0, prim1;
				double type_code;
				const auto vids = gather_full_vertices(cand, prim0, prim1, type_code);
				std::array<int, 4> point_idx_by_slot = { -1, -1, -1, -1 };
				Eigen::RowVectorXd pair_a = Eigen::RowVectorXd::Zero(problem_dim);
				Eigen::RowVectorXd pair_b = Eigen::RowVectorXd::Zero(problem_dim);
				...
				const double dist = std::sqrt(std::max(
					0.0,
					cand.compute_distance(
						displaced_surface, collision_mesh.edges(), collision_mesh.faces())));
				const double potential = std::numeric_limits<double>::quiet_NaN();
				const double is_active = 0.0;
				...
				{
					n_full_vv++;
					pair_a = displaced_surface.row(prim0).head(problem_dim);
					pair_b = displaced_surface.row(prim1).head(problem_dim);
					break;
				}
				case 2:
				{
				...
				pair_prim0(ci) = prim0;
				pair_prim1(ci) = prim1;
				pair_active_match(ci) = 0.0;
				pair_active_ev_count_at_vertex(ci) = 0.0;
				pair_active_ev_count_at_edge(ci) = 0.0;
				pair_active_vv_count_at_vertex(ci) = 0.0;
				if (int(type_code) == 0)
				{
					const int e = prim0;
					const int v = prim1;
					const long long key = (static_cast<long long>(e) << 32) | static_cast<unsigned int>(v);
					pair_active_match(ci) = active_ev_pairs.count(key) ? 1.0 : 0.0;
					const auto itv = active_ev_count_by_vertex.find(v);
					if (itv != active_ev_count_by_vertex.end())
						pair_active_ev_count_at_vertex(ci) = itv->second;
					const auto ite = active_ev_count_by_edge.find(e);
					if (ite != active_ev_count_by_edge.end())
						pair_active_ev_count_at_edge(ci) = ite->second;
					const auto itvv = active_vv_count_by_vertex.find(v);
					if (itvv != active_vv_count_by_vertex.end())
						pair_active_vv_count_at_vertex(ci) = itvv->second;
				}
			...
			for (int pi = 0; pi < n_points; ++pi)
				cand_cells(pi, 0) = pi;
			cand_writer.write_mesh(
				export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates_full.vtu",
				cand_pts, cand_cells);
			...
			pair_writer.add_cell_field("primitive0_id", pair_prim0);
			pair_writer.add_cell_field("primitive1_id", pair_prim1);
			pair_writer.add_cell_field("active_match", pair_active_match);
			pair_writer.add_cell_field("active_ev_count_at_vertex", pair_active_ev_count_at_vertex);
			pair_writer.add_cell_field("active_ev_count_at_edge", pair_active_ev_count_at_edge);
			pair_writer.add_cell_field("active_vv_count_at_vertex", pair_active_vv_count_at_vertex);
			pair_writer.write_mesh(
				export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates_full_pairs.vtu",
				pair_pts, pair_cells);
			logger().info("GCP full candidate pairs export: {}", n_candidates);

			auto write_full_pair_subset = [&](const int type_code, const std::string &suffix) {
				std::vector<int> selected;
				selected.reserve(n_candidates);
				for (int ci = 0; ci < n_candidates; ++ci)
				{
				...
				if (selected.empty())
				{
					logger().info("GCP full candidate pairs {} export: 0", suffix);
					return;
				}
				...
				Eigen::MatrixXi sub_cells(m, 2);
				Eigen::VectorXd sub_candidate_id(m), sub_type(m), sub_dist(m), sub_potential(m), sub_active(m), sub_prim0(m), sub_prim1(m);
				Eigen::VectorXd sub_active_match(m), sub_active_ev_count_at_vertex(m), sub_active_ev_count_at_edge(m), sub_active_vv_count_at_vertex(m);
				...
				for (int j = 0; j < m; ++j)
				{
					...
					sub_prim0(j) = pair_prim0(ci);
					sub_prim1(j) = pair_prim1(ci);
					sub_active_match(j) = pair_active_match(ci);
					sub_active_ev_count_at_vertex(j) = pair_active_ev_count_at_vertex(ci);
					sub_active_ev_count_at_edge(j) = pair_active_ev_count_at_edge(ci);
					sub_active_vv_count_at_vertex(j) = pair_active_vv_count_at_vertex(ci);
				}
				...
				sub_writer.add_cell_field("primitive0_id", sub_prim0);
				sub_writer.add_cell_field("primitive1_id", sub_prim1);
				sub_writer.add_cell_field("active_match", sub_active_match);
				sub_writer.add_cell_field("active_ev_count_at_vertex", sub_active_ev_count_at_vertex);
				sub_writer.add_cell_field("active_ev_count_at_edge", sub_active_ev_count_at_edge);
				sub_writer.add_cell_field("active_vv_count_at_vertex", sub_active_vv_count_at_vertex);
				sub_writer.write_mesh(
					export_surface.substr(0, export_surface.length() - 4) + suffix,
					sub_pts, sub_cells);
				logger().info("GCP full candidate pairs {} export: {}", suffix, m);
			};

			write_full_pair_subset(1, "_gcp_candidates_full_pairs_vv.vtu");
			write_full_pair_subset(0, "_gcp_candidates_full_pairs_ev.vtu");
			write_full_pair_subset(2, "_gcp_candidates_full_pairs_ee.vtu");
			write_full_pair_subset(3, "_gcp_candidates_full_pairs_fv.vtu");
			...
			if (!cand_line_cells.empty())
			{
				...
				cand_line_writer.add_cell_field("line_vertex_b", line_vertex_b);
				cand_line_writer.write_mesh(
					export_surface.substr(0, export_surface.length() - 4) + "_gcp_candidates_full_lines.vtu",
					cand_pts, line_cells);
				logger().info("GCP full candidate lines export: {}", n_lines);
			}
			logger().info(
				"GCP full types: vv={} ev={} ee={} fv={} other={}",
				n_full_vv, n_full_ev, n_full_ee, n_full_fv, n_full_other);
		}
