...
void ncMesh3D::global2Local(Eigen::MatrixXd& points, const Eigen::VectorXi& new_id) const
{
    assert(points.cols() == 3);
    Eigen::Matrix3d J;
    for (int i = 0; i < 3; i++)
        J.col(i) = vertices[new_id[i+1]].pos - vertices[new_id[0]].pos;
    ...
    {
        for (int j = 0; j < 3; j++)
            points(i, j) -= vertices[new_id[0]].pos(j);

        Eigen::Vector3d x = Jinv * points.row(i).transpose();
        points.row(i) = x;
    }
}

void ncMesh3D::global2Local(const int e, Eigen::MatrixXd& points) const
{
    global2Local(points, elements[e].vertices);
}

void ncMesh3D::local2Global(Eigen::MatrixXd& points, const Eigen::VectorXi& new_id) const
{
    assert(points.cols() == 3);
    ...
    for (int i = 0; i < N; i++)
    {
        points.row(i) = points(i, 0) * vertices[new_id[1]].pos + points(i, 1) * vertices[new_id[2]].pos + points(i, 2) * vertices[new_id[3]].pos + (1 - points(i, 0) - points(i, 1) - points(i, 2)) * vertices[new_id[0]].pos;
    }
}

void ncMesh3D::local2Global(const int e, Eigen::MatrixXd& points) const
{
    local2Global(points, elements[e].vertices);
}

void ncMesh3D::global2LocalEdge(const int edge, const Eigen::MatrixXd& points, Eigen::MatrixXd& uv) const
{
    const auto& v = edges[edge].vertices;

    uv.setZero(points.rows(), 1);
    for (int i = 0; i < points.rows(); i++)
        uv(i) = (points.row(i).transpose() - vertices[v(0)].pos).dot(vertices[v(1)].pos - vertices[v(0)].pos) / (vertices[v(1)].pos - vertices[v(0)].pos).squaredNorm();
}
void ncMesh3D::global2LocalFace(const int face, const Eigen::MatrixXd& points, Eigen::MatrixXd& uv) const
{
    const auto& v = faces[face].vertices;

    uv.setZero(points.rows(), 2);

    Eigen::Matrix3d J;
    for (int i = 0; i < 2; i++)
        J.col(i) = vertices[v(i+1)].pos - vertices[v(0)].pos;

    Eigen::Vector3d a = vertices[v(1)].pos - vertices[v(0)].pos;
    Eigen::Vector3d b = vertices[v(2)].pos - vertices[v(0)].pos;
    Eigen::Vector3d virtual_vert = a.cross(b);
    J.col(2) = virtual_vert - vertices[v(0)].pos;

    for (int i = 0; i < points.rows(); i++) {
        Eigen::Vector3d x = J.colPivHouseholderQr().solve(points.row(i).transpose() - vertices[v(0)].pos);
        uv.row(i) = x.block(0, 0, 2, 1).transpose();
        assert(std::abs(x(2)) < 1e-12);
    }
}
void ncMesh3D::local2GlobalEdge(const int edge, const Eigen::MatrixXd& uv, Eigen::MatrixXd& points) const
{
    const auto& v = edges[edge].vertices;

    points.setZero(uv.rows(), 3);
    for (int i = 0; i < points.rows(); i++)
        points.row(i) = uv(i) * vertices[v(1)].pos + (1 - uv(i)) * vertices[v(0)].pos;
}
void ncMesh3D::local2GlobalFace(const int face, const Eigen::MatrixXd& uv, Eigen::MatrixXd& points) const
{
    const auto& v = faces[face].vertices;

    points.setZero(uv.rows(), 3);
    for (int i = 0; i < points.rows(); i++)
        points.row(i) = (uv(i, 0) * vertices[v(1)].pos + uv(i, 1) * vertices[v(2)].pos + (1 - uv(i, 0) - uv(i, 1)) * vertices[v(0)].pos).transpose();
}

void ncMesh3D::isInside(const int e, const Eigen::MatrixXd& points, std::vector<bool>& is_inside) const
{
    Eigen::MatrixXd uvw = points;
    global2Local(e, uvw);
    is_inside.resize(points.rows());

    for (int i = 0; i < uvw.rows(); i++) {
        if (uvw(i, 0) > -1e-12 && uvw(i, 1) > -1e-12 && uvw(i, 2) > -1e-12 && uvw(i, 0) + uvw(i, 1) + uvw(i, 2) < 1+1e-12)
            is_inside[i] = true;
        else
            is_inside[i] = false;
    }
...
bool ncMesh3D::isInside(const int e, const Eigen::MatrixXd& point) const
{
    assert(point.rows() == 1 && point.cols() == 3);
    Eigen::MatrixXd uvw = point;
    global2Local(e, uvw);

    if (uvw(0) > -1e-12 && uvw(1) > -1e-12 && uvw(2) > -1e-12 && uvw(0) + uvw(1) + uvw(2) < 1+1e-12)
        return true;
    else
        return false;
}
...
int ncMesh3D::boundary_quadrature(const int ncelem_id, const int n_samples, Eigen::MatrixXd& local_pts, Eigen::MatrixXd& uv, Eigen::MatrixXd& normals, Eigen::VectorXd& weights, Eigen::VectorXi& face_ids) const
{
    Quadrature quad;
    ...
    v.row(2) << 0, 1, 0;
    v.row(3) << 0, 0, 1;
    Eigen::Matrix<int, 4, 4> fv_full;
    fv_full.row(0) << 0, 1, 2, 3;
    fv_full.row(1) << 0, 1, 3, 2;
    fv_full.row(2) << 1, 2, 3, 0;
    fv_full.row(3) << 2, 0, 3, 1;

    local_pts.setZero(n_pts*4, 3);
    uv.setZero(n_pts*4, 3);
    weights.setZero(n_pts*4);
    normals.setZero(n_pts*4, 3);
    face_ids.setZero(4);

    const auto& ncelem = elements[ncelem_id];
    for (int lf = 0; lf < 4; lf++) {
        Eigen::MatrixXi fv = fv_full.block(lf, 0, 1, 3);
        Eigen::MatrixXd endpoints(3, 3);
        face_ids(lf) = findFace(ncelem.vertices(fv(0)),ncelem.vertices(fv(1)),ncelem.vertices(fv(2)));
        for (int c = 0; c < 3; ++c)
            endpoints.row(c) = v.row(fv(c));

        for (int i = 0; i < n_pts; ++i) {
            const double b2 = quad.points(i, 0);
            const double b3 = quad.points(i, 1);
            const double b1 = 1 - b2 - b3;

            for (int c = 0; c < 3; ++c)
                local_pts(i + lf*n_pts, c) = b1 * endpoints(0, c) + b2 * endpoints(1, c) + b3 * endpoints(2, c);

            uv(i + lf*n_pts, 0) = quad.points(i, 0);
            uv(i + lf*n_pts, 1) = quad.points(i, 1);
            uv(i + lf*n_pts, 2) = 1.0 - quad.points(i, 0) - quad.points(i, 1);
        }

        weights.segment(lf*n_pts, n_pts) = quad.weights * (faceArea(face_ids(lf)) / quad.weights.sum());

        const Eigen::RowVector3d e1 = vertices[ncelem.vertices[fv(0)]].pos - vertices[ncelem.vertices[fv(1)]].pos;
        const Eigen::RowVector3d e2 = vertices[ncelem.vertices[fv(0)]].pos - vertices[ncelem.vertices[fv(2)]].pos;
        Eigen::RowVector3d normal = e1.cross(e2);
        normal.normalize();

        if (normal.dot((vertices[ncelem.vertices(fv_full(lf, 0))].pos - vertices[ncelem.vertices(fv_full(lf, 3))].pos).transpose()) < 0)
            normal *= -1;
...
int ncMesh3D::boundary_quadrature(const int ncelem_id, const int ncface_id, const int n_samples, Eigen::MatrixXd& local_pts, Eigen::MatrixXd& normals, Eigen::VectorXd& weights) const
{
    Quadrature quad;
    TriQuadrature quad_rule;
    quad_rule.get_quadrature(n_samples, quad);
    ...
    local_pts.setZero(n_pts, 3);
    weights.setZero(n_pts);
    normals.setZero(n_pts, 3);

    const auto& ncelem = elements[ncelem_id];
    const auto& ncface = faces[ncface_id];
    // find the vertex not on the face
    int other_vertex = -1;
    {
        std::vector<bool> flags(ncelem.vertices.size(), false);
        for (int i = 0; i < ncelem.vertices.size(); i++) {
            for (int j = 0; j < ncface.vertices.size(); j++) {
                if (ncelem.vertices[i] == ncface.vertices[j]) {
                    flags[i] = true;
                    break;
                }
            }
...

    {
        ...
        for (int i = 0; i < n_pts; ++i) {
            ...
            for (int c = 0; c < 3; ++c)
                local_pts(i, c) = b1 * endpoints(0, c) + b2 * endpoints(1, c) + b3 * endpoints(2, c);
        }

        weights = quad.weights * (faceArea(ncface_id) / quad.weights.sum());

        const Eigen::RowVector3d e1 = vertices[ncface.vertices[0]].pos - vertices[ncface.vertices[1]].pos;
        const Eigen::RowVector3d e2 = vertices[ncface.vertices[0]].pos - vertices[ncface.vertices[2]].pos;
        Eigen::RowVector3d normal = e1.cross(e2);
        normal.normalize();

        if (normal.dot((vertices[ncface.vertices[0]].pos - vertices[ncelem.vertices[other_vertex]].pos).transpose()) < 0)
            normal *= -1;
