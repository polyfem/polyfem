...
namespace polyfem
{
	...
	bool CMesh3D::save(const std::vector<int> &eles, const int ringN, const std::string &path) const
	{
		...
		for (int i = 0; i < ringN; i++)
		{
			...
			for (uint32_t j = 0; j < H_flag.size(); j++)
				if (H_flag[j])
				{
					for (const auto vid : mesh_.elements[j].vs)
						for (const auto nhid : mesh_.vertices[vid].neighbor_hs)
							H_flag_[nhid] = true;
				}
			H_flag = H_flag_;
		}
	...
	void CMesh3D::compute_body_ids(const std::function<int(const RowVectorNd &)> &marker)
	{
