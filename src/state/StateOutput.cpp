...
namespace polyfem
{
	...
	void State::save_surface(const std::string &export_surface)
	{
		...
		if ((export_contact_forces || export_friction_forces) && solve_export_to_file)
		{
			...
			writer.add_field("solution", displaced_surface);
			...
			if (export_contact_forces)
			{
				...
				assert(forces_reshaped.rows() == displaced_surface.rows());
				assert(forces_reshaped.cols() == displaced_surface.cols());
