...
namespace polyfem
{
    ...
    double State::get_drag_force(const RowVectorNd& center, const double radius)
    {
        if(!mesh->is_volume()) return 0;
        const Mesh3D &mesh3d = *dynamic_cast<Mesh3D *>(mesh.get());

        int n_boundary_faces = 0;
        for(int f = 0; f < mesh3d.n_faces(); f++)
        ...
        MatrixXi faces(n_boundary_faces, mesh3d.n_face_vertices(0));
        n_boundary_faces = 0;
        for(int f = 0; f < mesh3d.n_faces(); f++)
            if (mesh3d.is_boundary_face(f))
            {
                ...
                if ((point - center).norm() < radius)
                {
                    for (int p = 0; p < mesh3d.n_face_vertices(0); p++)
                        faces(n_boundary_faces, p) = mesh3d.face_vertex(f, p);
                    n_boundary_faces++;
                }
            }
        
        MatrixXd pts = MatrixXd::Zero(mesh3d.n_vertices(), 3);
        ...
        {
            RowVectorNd point = pts.block(faces(f, 0), 0, 1, 3) - center;
            RowVectorNd normal = normals.block(f, 0, 1, 3);
            //for (int p = 0; p < 3; p++)
            //{
            //    point(p) = pts(faces(f, 0), p) - center(p);
        ...
        {
            const ElementBases &gbs = gbases[e];
            const ElementBases &bs = pressure_bases[e];
